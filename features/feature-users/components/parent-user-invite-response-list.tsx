...
import { DataTable, EmptyPlaceholder, TableColumnInterface } from "@wso2is/react-components";
...
export const ParentInviteResponseList: React.FunctionComponent<ParentInviteResponseListProps> = (
    ...
    const { response, isLoading, hasError, ["data-componentid"]: componentId } = props;
        ...
        const defaultColumns: TableColumnInterface[] = [
            ...
            },
            {
                ...
                render: (item: ParentUserInviteResponseListItem): ReactNode => {
                    return (
                        <Header as="h6" data-componentid={`${componentId}-status-item-heading`}>
                            <Header.Content>{item.message}</Header.Content>
                        </Header>
                    );
                },
        ...
        <Grid>
            <Grid.Row columns={1}>
                <Grid.Column mobile={16} tablet={16} computer={16}>
                    {isLoading || hasError ? null : responseSummary?.failedCount === 0 ? (
                        ...
                        <Alert severity="error" data-componentid={`${componentId}-error-alert`}>
                            <AlertTitle data-componentid={`${componentId}-error-alert-title`}>
                                {t(
                                    "console:manage.features.user.modals.inviteParentUserWizard." + "errorAlert.message"
                                )}
                            </AlertTitle>
                            <Trans
                                i18nKey={
