...
export const GroupRolesV1List: FunctionComponent<GroupRolesV1PropsInterface> = (
    ...
    const { group, isReadOnly, onGroupUpdate } = props;
            ...
            getRolesList(null)
                ...
                .catch(() => {
                    dispatch(
                        addAlert({
                            description: t(
                                "console:manage.features.roles.notifications.fetchRoles." + "genericError.description"
                            ),
                ...
                .catch(() => {
                    dispatch(
                        addAlert({
                            ...
                            level: AlertLevels.ERROR,
                            message: t(
                                "console:manage.features.roles.notifications." + "fetchRoles.genericError.message"
                            )
                        })
                    );
                });
        ...
        updateResources(bulkData)
            .then(() => {
                dispatch(
                    addAlert({
                        description: t(
                            "console:manage.features.groups.notifications.updateGroup." + "success.description"
                        ),
            ...
            .catch((error: AxiosError) => {
                ...
                setAlert({
                    description: t(
                        "console:manage.features.groups.notifications.updateGroup.genericError." + "description"
                    ),
                    level: AlertLevels.ERROR,
                    message: t("console:manage.featuresgroups.notifications.updateGroup.genericError.message")
                });
            })
            .finally(() => {
    ...
    const removeRoles = () => {
        ...
        setCheckedAssignedListItems(
            checkedAssignedListItems.filter((x: RolesMemberInterface) => !removedRoles?.includes(x))
        );
        setCheckedUnassignedListItems([]);
                ...
                <Modal.Content image>
                    ...
                    >
                        ...
                        <TransferList
                            ...
                            emptyPlaceholderContent={t(
                                "console:manage.features.transferList.list." + "emptyPlaceholders.users.roles.selected",
                                { type: "roles" }
                            )}
                                                ...
                                                if (groupRole.length >= 1) {
                                                    return (
                                                        <Table.Row key={index}>
                                                            ...
                                                            <Table.Cell width={8}>
                                                                {groupRole.length == 1 ? groupRole[0] : groupRole[1]}
                                                            </Table.Cell>
