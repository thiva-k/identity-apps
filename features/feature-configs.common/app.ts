...
import { getApplicationsResourceEndpoints } from "../applications.common/configs/endpoints";
import isLegacyAuthzRuntime from "../authorization/utils/get-legacy-authz-runtime";
import { getBrandingResourceEndpoints } from "../branding.common/configs/endpoints";
import { getCertificatesResourceEndpoints } from "../certificates.common/configs/endpoints";
import { getClaimResourceEndpoints } from "../claims.common/configs/endpoints";
import { getConnectionResourceEndpoints } from "../connections.common/configs/endpoints";
import { getConsoleSettingsResourceEndpoints } from "../console-settings.common/configs/endpoints";
import { getEmailTemplatesResourceEndpoints } from "../email-templates.common/configs/endpoints";
import { I18nConstants, UIConstants } from "../feature-constants.common";
import { getGroupsResourceEndpoints } from "../feature-groups.common/configs/endpoints";
import { DeploymentConfigInterface, ServiceResourceEndpointsInterface,
     UIConfigInterface } from "../feature-models.common";
import { getOrganizationsResourceEndpoints } from "../feature-organizations.common/configs/endpoints";
import { OrganizationUtils } from "../feature-organizations.common/utils";
import { getRolesResourceEndpoints } from "../feature-roles.common/configs/endpoints";
import { store } from "../feature-store.common";
import { getUsersResourceEndpoints } from "../feature-users.common/configs/endpoints";
import { getIDPResourceEndpoints } from "../identity-providers.common/configs/endpoints";
import { getIDVPResourceEndpoints } from "../identity-verification-providers";
import { getScopesResourceEndpoints } from "../oidc-scopes.common/configs/endpoints";
import { getInsightsResourceEndpoints } from "../org-insights/config/org-insights";
import { getJWTAuthenticationServiceEndpoints } from "../private-key-jwt.common/configs/endpoints";
import { getRemoteFetchConfigResourceEndpoints } from "../remote-repository-configuration";
import { getSecretsManagementEndpoints } from "../secrets.common/configs/endpoints";
import { getServerConfigurationsResourceEndpoints } from "../server-configurations";
import { getTenantResourceEndpoints } from "../tenants.common/configs/endpoints";
import { getUserstoreResourceEndpoints } from "../userstores.common/configs/endpoints";
...
export class Config {
    ...
    public static resolveServerHostforFG(enforceOrgPath?: boolean): string {
        if (
            ...
            return `${window["AppUtils"]?.getConfig()?.serverOrigin}/t/${
                store.getState().organization.organization.id
            }`;
        }
    }
    ...
    public static getDeploymentConfig(): DeploymentConfigInterface {
        return {
            ...
            documentation: {
                baseURL:
                    window["AppUtils"]?.getConfig()?.documentation?.baseURL ??
                    DocumentationConstants.GITHUB_API_BASE_URL,
                contentBaseURL:
                    window["AppUtils"]?.getConfig()?.documentation?.contentBaseURL ??
                    DocumentationConstants.DEFAULT_CONTENT_BASE_URL,
                githubOptions: {
                    branch:
                        window["AppUtils"]?.getConfig()?.documentation?.githubOptions?.branch ??
                        DocumentationConstants.DEFAULT_BRANCH
                },
                imagePrefixURL:
                    window["AppUtils"]?.getConfig()?.documentation?.imagePrefixURL ??
                    DocumentationConstants.DEFAULT_IMAGE_PREFIX_URL,
                provider: window["AppUtils"]?.getConfig()?.documentation?.provider ?? DocumentationProviders.GITHUB,
                structureFileType:
                    window["AppUtils"]?.getConfig()?.documentation?.structureFileType ??
    ...
    public static getUIConfig(): UIConfigInterface {
        return {
            administratorRoleDisplayName:
                window["AppUtils"]?.getConfig()?.ui?.administratorRoleDisplayName ??
                UIConstants.ADMINISTRATOR_ROLE_DISPLAY_NAME,
            announcements: window["AppUtils"]?.getConfig()?.ui?.announcements,
            appCopyright: window["AppUtils"]
                ?.getConfig()
            ...
            identityProviderTemplateLoadingStrategy: window["AppUtils"]?.getConfig()?.ui
