...
export const AttributeSelectionOIDC: FunctionComponent<AttributeSelectionOIDCPropsInterface> = (
    ...
    const handleSelectedScopeCheckChange = (
        ...
        value: { value: string; checked: boolean }
    ): void => {
        const tempExternalClaimsGroupedByScopes: OIDCScopesClaimsListInterface[] = [
            ...unfilteredExternalClaimsGroupedByScopes
        ];

        tempExternalClaimsGroupedByScopes
            .find((scope: OIDCScopesClaimsListInterface) => {
    ...
    const checkInitialRequestMandatory = (uri: string): boolean => {
        ...
        requestURI = claimConfigurations.requestedClaims.find(
            (requestClaims: RequestedClaimConfigurationInterface) => requestClaims?.claim?.uri === uri
        )?.mandatory;
    ...
    const searchFilter = (changeValue: string) => {
        if (changeValue !== "") {
            const scopesFiltered: OIDCScopesClaimsListInterface[] = unfilteredExternalClaimsGroupedByScopes.filter(
                (item: OIDCScopesClaimsListInterface) =>
            ...
            unfilteredClaims.forEach((scope: OIDCScopesClaimsListInterface) => {
                const matchedClaims: ExtendedExternalClaimInterface[] = scope.claims.filter(
            ...
            >
                ...
                {scope.name === "" ? (
                    <Header.Content className="align-self-center">
                        ...
                        {scope.displayName}
                        <Header.Subheader>{" " + (scope.description ?? "")}</Header.Subheader>
                    </Header.Content>
                ) : (
                    <Header.Content>
                        {scope.displayName}
    ...
    const resolveUserAttributeList = (claimsGroupedByScopes: ExtendedExternalClaimInterface[]): ReactElement => {
        return (
            <Table singleLine compact data-testid={`${testId}-list`} data-componentId={`${componentId}-list`} fixed>
                        ...
                        {claimsGroupedByScopes.map((claim: ExtendedExternalClaimInterface) => {
                            return (
                                <AttributeListItem
                                    ...
                                    initialMandatory={
                                        selectedSubjectValue === claim.mappedLocalClaimURI && !onlyOIDCConfigured
                                            ? true
                                            : claim.mandatory
                                    }
                                    ...
                                    subject={selectedSubjectValue === claim.mappedLocalClaimURI && !onlyOIDCConfigured}
    ...
    const handleAccordionTitleClick = (scope: OIDCScopesClaimsListInterface) => {
        ...
        if (!expandedScopes?.includes(scope.name)) {
            ...
            tempExpandedScopes = tempExpandedScopes.filter((scopeDeselected: string) => scopeDeselected !== scope.name);
        }
        setExpandedScopes(tempExpandedScopes);
    };

    const createAccordionTitleAction = (
    ...
    return !isUserAttributesLoading && claimConfigurations && initializationFinished ? (
        <>
                ...
                <Grid.Column computer={16} tablet={16} largeScreen={12} widescreen={12}>
                        ...
                        <Grid.Row className="mb-5">
                            {externalClaimsGroupedByScopes.length !== 0 ? (
                                <SegmentedAccordion
                                    ...
                                    viewType="table-view"
                                >
                                    {externalClaimsGroupedByScopes.map((scope: OIDCScopesClaimsListInterface) => {
                                        if (scope.name !== "") {
                                            return (
                                                <Fragment key={scope.name}>
                                                    <SegmentedAccordion.Title
                                                        ...
                                                        data-componentid={`${componentId}-${scope.name}
                                                                                    -title`}
                                                        active={expandedScopes?.includes(scope.name) || false}
                                                        accordionIndex={scope.name}
                                                        onClick={() => handleAccordionTitleClick(scope)}
                                                        content={resolveScopeListItem(scope)}
                                                        hideChevron={false}
                    ...
                    <Hint>
                        ...
                        >
                            ...
                            to manage user attribute to a scope. You can add new attributes by navigating to
                            <Link
                                external={false}
                                onClick={() => {
                                    history.push(
            ...
            <Hint>
                ...
                >
                    Request these scopes from your application to retrieve the user attributes you selected.
                </Trans>
                {getLink("develop.applications.editApplication.attributeManagement" + ".manageOIDCScopes") && (
                    <DocumentationLink
                        link={getLink("develop.applications.editApplication.attributeManagement" + ".manageOIDCScopes")}
                    >
                        {t(
                            "console:develop.features.applications.edit.sections.attributes." +
