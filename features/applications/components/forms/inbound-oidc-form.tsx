...
import { ConfigReducerStateInterface } from "@wso2is/feature-models.common/reducer-state";
import { getSharedOrganizations } from "@wso2is/feature-organizations.common/api";
...
import OIDCWebApplicationTemplate from "../../data/application-templates/templates/oidc-web-application/oidc-web-application.json";
...
export const InboundOIDCForm: FunctionComponent<InboundOIDCFormPropsInterface> = (
    ...
    const [isTokenBindingTypeSelected, setIsTokenBindingTypeSelected] = useState<boolean>(false);
    ...
    const [subjectDN, setTLSClientAuthSubjectDN] = useState(
        initialValues?.clientAuthentication ? initialValues.clientAuthentication.tlsClientAuthSubjectDn : null
    );
    const [selectedAuthMethod, setSelectedAuthMethod] = useState<string>(undefined);
    const [isRefreshTokenWithoutAllowedGrantType, setRefreshTokenWithoutAlllowdGrantType] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        if (
            sharedOrganizationsList?.length > 0 ||
            (isSharedWithAll?.length > 0 && JSON.parse(isSharedWithAll[0].value))
        ) {
            setIsAppShared(true);
        }
    ...
    useEffect(() => {
        ...
        const selectedRefreshTokenAllowedGrantTypes: string[] = intersection(
            ApplicationManagementConstants.IS_REFRESH_TOKEN_GRANT_TYPE_ALLOWED,
            selectedGrantTypes
        );
    ...
    const isHideRefreshTokenGrantType = (selectedGrantTypes: string[]): boolean => {
        if (
            ...
            (selectedGrantTypes?.includes("implicit") && selectedGrantTypes?.length === 1)
        ) {
            return true;
        }
        if (
                ...
                metadataProp.options.map((ele: any, index: number) => {
                    if (!ele.displayName) {
                        allowedList.push({
                            ...
                            value:
                                ele !==
                                t("console:develop.features.applications" + ".forms.inboundOIDC.dropdowns.selectOption")
                                    ? ele
                                    : ""
                        });
                    } else {
    ...
    const getAllowedListForAccessToken = (
        metadataProp: MetadataPropertyInterface,
        isBinding?: boolean
    ): RadioChild[] => {
    ...
    const modifyGrantTypeLabels = (value: string, label: string) => {
        if (
            value === ApplicationManagementConstants.IMPLICIT_GRANT ||
    ...
    const getGrantTypeHintDescription = (value: string): string => {
        switch (value) {
            case ApplicationManagementConstants.IMPLICIT_GRANT:
                return t(
                    "console:develop.features.applications.forms.inboundOIDC.fields.grant.children." + "implicit.hint"
                );
            case ApplicationManagementConstants.PASSWORD:
                return t(
                    "console:develop.features.applications.forms.inboundOIDC.fields.grant.children." + "password.hint"
                );
            ...
            case ApplicationManagementConstants.REFRESH_TOKEN_GRANT:
                return t(
                    "console:develop.features.applications.forms.inboundOIDC.fields.grant.validation." + "refreshToken"
                );
            default:
                return null;
        }
    ...
    const getAllowedGranTypeList = (metadataProp: GrantTypeMetaDataInterface): CheckboxChild[] => {
        ...
        if (metadataProp) {
            metadataProp.options.map(({ name, displayName }: GrantTypeInterface) => {
                ...
                if (
                    name === ApplicationManagementConstants.ORGANIZATION_SWITCH_GRANT &&
        ...
        if (template && template.id) {
            const arrangement: Map<string, number> =
                ApplicationManagementConstants.TEMPLATE_WISE_ALLOWED_GRANT_TYPE_ARRANGE_ORDER[template.id];
        ...
        if (
            applicationConfig.inboundOIDCForm.disabledGrantTypes &&
    ...
    const getRevokeStateForSPA = (values: any): boolean => {
        return values.get("RevokeAccessToken")
            ? values.get("RevokeAccessToken")?.length > 0
        ...
        if (!isSystemApplication && !isDefaultApplication) {
            let inboundConfigFormValues: any = {
                ...
                idToken: {
                    ...
                    encryption: {
                        algorithm:
                            isEncryptionEnabled && isCertAvailableForEncrypt
            ...
            !applicationConfig.inboundOIDCForm.showScopeValidators && delete inboundConfigFormValues.scopeValidators;
            ...
            finalConfiguration = {
                general: {
                    advancedConfigurations: {
                        certificate: {
                            type:
                                selectedCertType !== CertificateTypeInterface.NONE
                                    ? selectedCertType
                                    : certificate?.type,
            ...
            if (showCallbackURLField) {
                finalConfiguration.inbound.allowedOrigins = ApplicationManagementUtils.resolveAllowedOrigins(
    ...
    */
    ...
    const renderOIDCConfigFields = (): ReactElement => (
        <>
            {!readOnly && (
                <StickyBar updateButtonRef={updateRef} isFormStale={isFormStale} containerRef={containerRef}>
                    <Button
                        primary
                        type="submit"
                        size="small"
                ...
                <Grid.Row columns={2}>
                    <Grid.Column mobile={16} tablet={16} computer={16}>
                        <Field
                            ref={grant}
                            name="grant"
                            label={t("console:develop.features.applications.forms.inboundOIDC.fields.grant.label")}
                            type="checkbox"
                            required={true}
                            requiredErrorMessage={t(
                                "console:develop.features.applications.forms.inboundOIDC.fields.grant" +
                                    ".validations.empty"
                            )}
                        ...
                        <Hint>{t("console:develop.features.applications.forms.inboundOIDC.fields.grant.hint")}</Hint>
                    </Grid.Column>
                </Grid.Row>
            )}
                ...
                <>
                            ...
                            <URLInput
                                ...
                                validationErrorMsg={
                                    CustomApplicationTemplate?.id !== template?.id && !isMobileApplication
                                        ...
                                        : t(
                                              "console:develop.features.applications.forms.inboundOIDC.messages." +
                                ...
                                skipInternalValidation={template.templateId === ApplicationManagementConstants.MOBILE}
                    ...
                    <Grid.Row columns={1}>
                        <Grid.Column mobile={16} tablet={16} computer={16} className="field">
                            ...
                            />
                            <Hint>
                                The HTTP origins that host your {!isMobileApplication && "web"} application. You can
                                define multiple web origins by adding them separately.
                                <p className={"mt-0"}>
                                    (E.g.,&nbsp;&nbsp;
            ...
            {/* Form Section: PKCE */}
            {showPKCEField && !isSystemApplication && !isDefaultApplication && (
                <>
                    <Grid.Row columns={2}>
                        <Grid.Column mobile={16} tablet={16} computer={16}>
                            <Divider />
                        ...
                        <Grid.Column mobile={16} tablet={16} computer={16}>
                            <Heading as="h4">
                                {t(
                                    "console:develop.features.applications.forms.inboundOIDC.sections.pkce" + ".heading"
                                )}
                            </Heading>
                            <Field
                                ref={pkce}
                                ...
                                required={false}
                                requiredErrorMessage={t(
                                    "console:develop.features.applications.forms.inboundOIDC.sections.pkce" +
                                            ...
                                            <Grid.Column mobile={16} tablet={16} computer={16}>
                                                <Field
                                                    ...
                                                    listen={(values: Map<string, FormValue>): void => {
                                                        const isPublicClient: boolean = values
                                                            .get("supportPublicClients")
            ...
            {!isM2MApplication && !isSystemApplication && !isDefaultApplication && (
                <Grid.Row columns={1}>
                    <Grid.Column mobile={16} tablet={16} computer={16}>
                        <Field
                            ref={bindingType}
                            ...
                            default={
                                initialValues?.accessToken?.bindingType
                                    ? initialValues.accessToken.bindingType
                                    : isFAPIApplication
                                    ? SupportedAccessTokenBindingTypes.CERTIFICATE
                        ...
                        <Hint>
                            ...
                            >
                                ...
                                to the login session and issue a new token per session. When the application session
                                ends, the tokens will also be revoked.
                            </Trans>
                        </Hint>
                    </Grid.Column>
                </Grid.Row>
                        ...
                        <Grid.Row columns={1}>
                            <Grid.Column mobile={16} tablet={16} computer={16}>
                                <Field
                                    ...
                                    type="checkbox"
                                    value={isValidateTokenBindingEnabled() ? ["validateTokenBinding"] : []}
                                    children={[
                                        {
                                            label: t(
                                                "console:develop.features.applications.forms.inboundOIDC" +
            ...
            {selectedGrantTypes?.includes("client_credentials") && !isSystemApplication && !isDefaultApplication && (
                <>
                    <Grid.Row columns={1}>
                        <Grid.Column mobile={16} tablet={16} computer={16}>
                            <Field
                        ...
                        <URLInput
                            ...
                            hint={
                                ...
                                >
                                    ...
                                    intended for. By default, the client ID of this application is added as an audience.
                                </Trans>
                            }
                            readOnly={readOnly}
                            addURLTooltip={t("common:addURL")}
                            ...
                            showPredictions={false}
                            popupHeaderPositive={t("console:develop.features.URLInput.withLabel." + "positive.header")}
                            popupHeaderNegative={t("console:develop.features.URLInput.withLabel." + "negative.header")}
                            popupContentPositive={t(
                                "console:develop.features.URLInput.withLabel." + "positive.content",
                                { productName: config.ui.productName }
                            )}
            ...
            {/* Refresh Token */}
            {selectedGrantTypes?.includes("refresh_token") && !isSystemApplication && !isDefaultApplication && (
                <>
                    <Grid.Row columns={2}>
                        <Grid.Column mobile={16} tablet={16} computer={16}>
                            <Divider />
                        ...
                        <Grid.Column mobile={16} tablet={16} computer={16}>
                            ...
                            <Hint>
                                ...
                                >
                                    ...
                                    each time a <Code withBackground>refresh_token</Code> is exchanged. The existing
            ...
            {!isM2MApplication && !isSystemApplication && !isDefaultApplication && (
                <Grid.Row columns={2}>
                    ...
                    <Grid.Column mobile={16} tablet={16} computer={16} className="field">
                        <Heading as="h4">
                            {t("console:develop.features.applications.forms.inboundOIDC.sections" + ".idToken.heading")}
                        </Heading>
                        <URLInput
                            isAllowEnabled={false}
                            tenantDomain={tenantDomain}
                        ...
                        <Grid.Row columns={1}>
                            <Grid.Column mobile={16} tablet={16} computer={16}>
                                <Field
                                    ...
                                    type="checkbox"
                                    listen={(values: Map<string, FormValue>): void => {
                                        const encryptionEnabled: boolean = values
                                            .get("encryption")
                                ...
                                <Hint>
                                    ...
                                    >
                                        ...
                                        token using the public key of your application. To use encryption, configure the
                                        JWKS endpoint or the certificate of your application in the Certificate section
                                        below.
                                    </Trans>
                                </Hint>
                            </Grid.Column>
                    ...
                    <Grid.Row columns={1}>
                        <Grid.Column mobile={16} tablet={16} computer={16}>
                            <Field
                                ...
                                type="dropdown"
                                disabled={false}
                                default={initialValues?.idToken ? initialValues.idToken.idTokenSignedResponseAlg : null}
                                placeholder={t(
                                    "console:develop.features.applications.forms.inboundOIDC.sections" +
            ...
            {applicationConfig.inboundOIDCForm.showFrontChannelLogout && !isSystemApplication && !isDefaultApplication && (
                <Grid.Row columns={1}>
                    <Grid.Column mobile={16} tablet={16} computer={16}>
                        <Field
                        ...
                        <Grid.Row columns={2}>
                            ...
                            <Grid.Column mobile={16} tablet={16} computer={16}>
                                ...
                                <Hint>
                                    ...
                                    >
                                        WSO2 Identity Server supports receiving an OIDC authentication request as a
                                        request object that is passed in a single, self-contained request parameter.
                                        Enable signature validation to accept only signed
            ...
            {/* Scope Validators */}
            {applicationConfig.inboundOIDCForm.showScopeValidators && !isSystemApplication && !isDefaultApplication && (
                <Grid.Row columns={2}>
                    <Grid.Column mobile={16} tablet={16} computer={16}>
                        <Divider />
                        <Divider hidden />
    ...
    const renderRegenerateConfirmationModal = (): ReactElement => (
        <ConfirmationModal
            ...
            assertionHint={
                <p>
                    <Trans
                        i18nKey={
                            "console:develop.features.applications.confirmations" + ".regenerateSecret.assertionHint"
                        }
        ...
        >
            <ConfirmationModal.Header data-testid={`${testId}-oidc-regenerate-confirmation-modal-header`}>
                {t("console:develop.features.applications.confirmations" + ".regenerateSecret.header")}
            ...
            >
                {t("console:develop.features.applications.confirmations" + ".regenerateSecret.message")}
            </ConfirmationModal.Message>
            <ConfirmationModal.Content data-testid={`${testId}-oidc-regenerate-confirmation-modal-content`}>
                {t("console:develop.features.applications.confirmations" + ".regenerateSecret.content")}
        ...
        <ConfirmationModal
            ...
            assertionHint={
                <p>
                    <Trans
                        i18nKey={
                            "console:develop.features.applications.confirmations" + ".revokeApplication.assertionHint"
                        }
                        tOptions={{ id: initialValues?.clientId }}
                    >
                        Please type <strong>{initialValues?.clientId}</strong> to confirm.
            ...
            closeOnDimmerClick={false}
        >
            <ConfirmationModal.Header data-testid={`${testId}-oidc-revoke-confirmation-modal-header`}>
                {t("console:develop.features.applications.confirmations" + ".revokeApplication.header")}
            </ConfirmationModal.Header>
            <ConfirmationModal.Message
                attached
                warning
            ...
            >
                {t("console:develop.features.applications.confirmations" + ".revokeApplication.message")}
            </ConfirmationModal.Message>
            <ConfirmationModal.Content data-testid={`${testId}-oidc-revoke-confirmation-modal-content`}>
                {isSPAApplication
                    ? t("console:develop.features.applications.confirmations" + ".revokeApplication.content")
                    : null}
            </ConfirmationModal.Content>
        </ConfirmationModal>
    );
    ...
    const renderReactivateConfirmationModal = (): ReactElement => {
        return (
            <ConfirmationModal
                ...
                primaryAction={t("common:confirm")}
                secondaryAction={t("common:cancel")}
                onSecondaryActionClick={(): void => setShowReactiveConfirmationModal(false)}
                onPrimaryActionClick={(): void => {
                    onApplicationRegenerate();
                ...
                closeOnDimmerClick={false}
            >
                <ConfirmationModal.Header data-testid={`${testId}-oidc-reactivate-confirmation-modal-header`}>
                    {!isSPAApplication
                        ? t("console:develop.features.applications.confirmations" + ".reactivateSPA.header")
                        : t("console:develop.features.applications.confirmations" + ".reactivateOIDC.header")}
                </ConfirmationModal.Header>
                {isSPAApplication ? (
                    <ConfirmationModal.Message
                        attached
                    ...
                    >
                        {t("console:develop.features.applications.confirmations" + ".reactivateSPA.message")}
                    </ConfirmationModal.Message>
                ) : null}
                <ConfirmationModal.Content data-testid={`${testId}-oidc-reactivate-confirmation-modal-content`}>
                    {isSPAApplication
                        ? t("console:develop.features.applications.confirmations" + ".reactivateSPA.content")
                        : t("console:develop.features.applications.confirmations" + ".reactivateOIDC.content")}
                </ConfirmationModal.Content>
            </ConfirmationModal>
        );
    };
    ...
    const isExpiryTimesTooLow = (values: Map<string, FormValue>, url?: string, origin?: string): boolean => {
        const isUserAccessTokenExpiryInSecondsTooLow: boolean =
            parseInt(values.get("userAccessTokenExpiryInSeconds") as string, 10) < 60;
        ...
        if (isUserAccessTokenExpiryInSecondsTooLow || isExpiryInSecondsTooLow || isIdExpiryInSecondsTooLow) {
            ...
            setLowExpiryTimesConfirmationModal(
                <ConfirmationModal
                    ...
                    closeOnDimmerClick={false}
                >
                    <ConfirmationModal.Header data-testid={`${testId}-low-expiry-times-confirmation-modal-header`}>
                        {t("console:develop.features.applications.confirmations" + ".lowOIDCExpiryTimes.header")}
                    </ConfirmationModal.Header>
                    <ConfirmationModal.Message
                        attached
                        warning
                    ...
                    >
                        {t("console:develop.features.applications.confirmations" + ".lowOIDCExpiryTimes.message")}
                    </ConfirmationModal.Message>
                    <ConfirmationModal.Content data-testid={`${testId}-low-expiry-times-confirmation-modal-content`}>
                        <Text>
                        ...
                        <Text>
                            {t(
                                "console:develop.features.applications.confirmations.lowOIDCExpiryTimes." +
                                    "assertionHint"
                            )}
                        </Text>
                    </ConfirmationModal.Content>
                                    ...
                                    <Message
                                        ...
                                        content={
                                            ...
                                            >
                                                ...
                                                <Code withBackground>client_secret</Code> to native applications or web
                                                browser-based applications for the purpose of client authentication.
                                            </Trans>
                                        }
                                    />
                                ) : null}
