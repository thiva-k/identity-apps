...
export const RoleGroupsList: FunctionComponent<RoleGroupsPropsInterface> = (
    ...
    const { role, onRoleUpdate, isReadOnly } = props;
    ...
    useEffect(() => {
        getGroupList(null).then((response: GroupListInterface | any) => {
    ...
    const handleSelectedListSearch = (_e: FormEvent<HTMLInputElement>, { value }: { value: string }) => {
        ...
        if (!isEmpty(value)) {
            ...
            tempGroupList &&
                tempGroupList?.map((group: GroupsInterface) => {
                    isMatch = groupListFilterRegExp.test(group.displayName);
                    if (isMatch) {
                        filteredGroupList.push(group);
                ...
                >
                    <TransferList
                        ...
                        emptyPlaceholderContent={t(
                            "console:manage.features.transferList.list." + "emptyPlaceholders.roles.unselected",
                            { type: "groups" }
                        )}
    ...
    const handleAssignedGroupListSearch = (_e: FormEvent<HTMLInputElement>, { value }: { value: string }) => {
        ...
        if (!isEmpty(value)) {
            ...
            assignedGroups &&
                assignedGroups?.map((group: RoleGroupsInterface) => {
                    const groupName: string[] = group.display.split("/");
                    ...
                    if (userGroup[0] !== APPLICATION_DOMAIN && userGroup[0] !== INTERNAL_DOMAIN) {
                        return (
                            <Table.Row key={index}>
                                <Table.Cell>
                                    {userGroup?.length === 1 ? (
                                        <Label color="olive">{PRIMARY_DOMAIN}</Label>
                                    ) : (
                                        <Label color="olive">{userGroup[0]}</Label>
                                    )}
                                </Table.Cell>
                        ...
                        {primaryGroupsList?.size > 0 ? (
                            ...
                            <EmphasizedSegment>
                                <EmptyPlaceholder
                                    ...
                                    title={t(
                                        "console:manage.features.roles.edit.groups.placeholders." +
                                    ...
                                    action={
                                        !isReadOnly && (
                                            ...
                                            >
                                                {t(
                                                    "console:manage.features.roles.edit.groups." +
