...
export class RoleManagementUtils {
    ...
    public static validateRoleName = (roleName: string): Promise<boolean> => {
        ...
        return searchRoleList(searchData).then((response: AxiosResponse) => response?.data?.totalResults === 0);
    };
    ...
    public static getDisplayName = (nameWithUserstore: string): string =>
        nameWithUserstore?.split("/").length > 1 ? nameWithUserstore?.split("/")[1] : nameWithUserstore;
