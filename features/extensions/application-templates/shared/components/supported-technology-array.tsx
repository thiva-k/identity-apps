...
import { DocumentationLink, GenericIcon, GenericIconProps, Heading, Popup, Text } from "@wso2is/react-components";
...
export const SupportedTechnologyArray: FC<SupportedTechnologyArrayPropsInterface> = (
    ...
    const { icons, onTriggerTabUpdate, documentationLink, infoTabIndex, isMobileAppQuickstart } = props;
        ...
        >
            ...
            <Card.Content className="custom-config-message">
                <Heading as="h6" textAlign="center" compact className={isMobileAppQuickstart ? "mt-3" : "mt-0"}>
                    <Trans
                        i18nKey={"extensions:console.application.quickStart" + ".technologySelectionWrapper.subHeading"}
                    >
                        Use the{" "}
                        <a className="link pointing" onClick={onServerEndpointConfigTabClick}>
                            server endpoint details
                        </a>{" "}
                        and start integrating your own app or read through our{" "}
                        <DocumentationLink link={documentationLink}>documentation</DocumentationLink> to learn more.
                    </Trans>
                </Heading>
            </Card.Content>
        </Card>
