...
export const AddUserWizardSummary: FunctionComponent<AddUserWizardSummaryProps> = (
    ...
    const { selectedUserStore, username, password, isPasswordBased, ["data-componentid"]: componentId } = props;

    const { t } = useTranslation();
    const dispatch: Dispatch = useDispatch();

    const tenantDomain: string = useSelector((state: AppState) => state.auth.tenantDomain);
    const currentOrganization: string = useSelector((state: AppState) => state?.organization?.currentOrganization);
        ...
        generateInviteLink(username, selectedUserStore)
            ...
            .catch((error: AxiosError) => {
                ...
                if (!error.response || error.response.status === 401) {
                    dispatch(
                        addAlert({
                            description: t("extensions:manage.features.user.addUser.inviteLink.error.description"),
                            level: AlertLevels.ERROR,
                            message: t("extensions:manage.features.user.addUser.inviteLink.error.message")
                        })
                    );
                    ...
                    dispatch(
                        addAlert({
                            ...
                            level: AlertLevels.ERROR,
                            message: t("extensions:manage.features.user.addUser.inviteLink.genericError.message")
                        })
                    );
                }
            })
    ...
    const copyInvitation = () => {
        if (isPasswordBased) {
            const invitationBody: string =
                t("extensions:manage.features.user.addUser.summary.invitationBody.hi") +
                "\n\n" +
                t(
                    "extensions:manage.features.user.addUser.summary.invitationPasswordBodyCopy.accountHasBeenCreated"
                ).replace("$tenantname", tenantname) +
                "\n\n" +
                t("extensions:manage.features.user.addUser.summary.username") +
                `: ${username}\n` +
                t("extensions:manage.features.user.addUser.summary.password") +
                `: ${password}\n\n` +
                t("extensions:manage.features.user.addUser.summary.invitationBody.thanks") +
                ",\n" +
                t("extensions:manage.features.user.addUser.summary.invitationBodyCopy.team").replace(
                    "$tenantname",
                    tenantname
                );
                ...
                <Grid.Column mobile={16} tablet={16} computer={16}>
                    ...
                    <Grid className="segment mt-2">
                        <Grid.Column width={14}>
                            ...
                            >
                                An account has been created for you in the {tenantname} organization. Your credentials
                                are as follows.
                            </Trans>
                            <br />
                            <br />
                            ...
                            <br />
                            <Trans
                                i18nKey={"extensions:manage.features.user.addUser.summary.invitationBody." + "team"}
                                tOptions={{
                                    tenantname
                                }}
                            >
                        ...
                        <Grid.Column width={2}>
                            <Popup
                                ...
                                position="top center"
                                trigger={<Button icon="copy" floated="right" onClick={() => copyInvitation()} />}
                                inverted
                ...
                <Grid.Column mobile={16} tablet={16} computer={16}>
                    <Message className="mb-5" warning>
                        <Grid>
                            ...
                            <Grid.Column width={15} className="pt-0 pb-0">
                                {t("extensions:manage.features.user.addUser.summary." + "inviteWarningMessage")}
                            </Grid.Column>
                        </Grid>
                    </Message>
