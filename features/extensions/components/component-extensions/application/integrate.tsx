...
import { getInboundProtocolConfig } from "../../../../applications/api";
import { ApplicationInterface, SupportedAuthProtocolTypes } from "../../../../applications/models";
import { useDispatch } from "react-redux";
import { useTranslation } from "react-i18next";
import { store, toggleHelpPanelVisibility, AppState } from "../../../../core";
...
export const IntegrateAppComponent: FunctionComponent<IntegrateAppComponentProps> = (
    ...
    const { applicationType, application, ["data-testid"]: testId } = props;
            ...
            getInboundProtocolConfig(application.id, protocolName)
                ...
                .finally(() => {
                    ...
                    const urlArray = protocolConfigs?.oidc
                        ? EncodeDecodeUtils.decodeURLRegex(protocolConfigs?.oidc?.callbackURLs[0])
                        : undefined;

                    if (urlArray && urlArray.length > 1) {
                        urlArray.forEach(callbackUrl => {
                            setCallBackUrlSelection(prevItems => [
                                ...prevItems,
                                {
                                    key: callbackUrl,
                                    text: callbackUrl,
...
// Response with basic user details upon logged in
    ...
    const webAppContent = [
        {
            stepTitle: `Select Application Type ${
                selectedWebAppTechnology ? "(" + selectedWebAppTechnology + ")" : ""
            }`,
            stepContent: (
                <>
                    ...
                    <Card.Group itemsPerRow={helpPanelVisibilityGlobalState ? 5 : 9}>
                        <Card
                            ...
                            className={`basic-card tech-selection ${
                                selectedWebAppTechnology === "React" ? "selected" : ""
                            } `}
                            onClick={() => {
                        ...
                        >
                            <Card.Content textAlign={"center"}>
                                <ReactIcon />
                                <Card.Description>ReactJS</Card.Description>
                            </Card.Content>
                        </Card>
                        <Card
                            raised={false}
                            ...
                            className={`basic-card tech-selection ${
                                selectedWebAppTechnology === "Angular" ? "selected" : ""
                            } `}
                            onClick={() => {
                        ...
                        >
                            <Card.Content textAlign={"center"}>
                                <AngularIcon />
                                <Card.Description>Angular</Card.Description>
                            </Card.Content>
                        </Card>
                    </Card.Group>
                </>
                ...
                <>
                    ...
                    <div className="code-segment">
                        {selectedWebAppTechnology === "React" && (
                            <>
                                <div data-testid={`${testId}-react-config`} className="action-buttons">
                        ...
                        {selectedWebAppTechnology === "Angular" && (
                            <>
                                <div data-testid={`${testId}-angular-config`} className="action-buttons">
                                    <Button basic size="tiny" className="copy-config" icon>
                                        <Icon name="copy" />
                                    </Button>
                                    <Button
...
<script>
    ...
    return applicationType === "Web Application" ? (
        <VerticalStepper stepContent={webPageContent} isSidePanelOpen />
    ) : (
        <VerticalStepper stepContent={webAppContent} isNextEnabled={selectedWebAppTechnology !== ""} isSidePanelOpen />
    );
};
