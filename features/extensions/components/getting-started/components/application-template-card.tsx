...
export const ApplicationTemplateCard: FC<DynamicApplicationContextCardPropsInterface> = (
        ...
        context === "RECENT_APPS" ? (
            <ApplicationAddTile
                ...
                onClick={(e: MouseEvent) => {
                    ...
                    setContext(context === "RECENT_APPS" ? "TEMPLATES" : "RECENT_APPS");
                }}
            />
        ) : (
            <CategoryItem
    ...
    const tile2: ReactElement =
        context === "RECENT_APPS" ? (
            <div
                className={classNames("ui scale transition visible", {
                    "animating in visible flex-layout": context === "RECENT_APPS",
                    "hidden animating out": context !== "RECENT_APPS"
                })}
            >
                <ApplicationTile data-componentid={`${testId}-app-list-item-1`} application={applications[0]} />
            </div>
        ) : (
            <CategoryItem
    ...
    const tile3: ReactElement =
        context === "RECENT_APPS" ? (
            ...
            >
                <ApplicationTile data-componentid={`${testId}-app-list-item-2`} application={applications[1]} />
            </div>
        ) : (
            <CategoryItem
    ...
    const tile4: ReactElement =
        context === "RECENT_APPS" ? (
            ...
            >
                <ApplicationTile data-componentid={`${testId}-app-list-item-3`} application={applications[2]} />
            </div>
        ) : (
            <CategoryItem
            ...
            <Card.Content className="pt-0 px-2 no-borders">
                <Grid>
                    <Grid.Row columns={2} style={{ rowGap: "20px" }}>
                        {[tile1, tile2, tile3, tile4].map(
                            (tile: ReactElement<any, string | JSXElementConstructor<any>>, index: number) => (
