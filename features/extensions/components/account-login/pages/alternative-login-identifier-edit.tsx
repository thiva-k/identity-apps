...
export const AlternativeLoginIdentifierEditPage: FunctionComponent<AlternativeLoginIdentifierEditPage> = (
    ...
    const initializeForm = (): void => {
        ...
        const isEnabled: boolean =
            connector?.properties?.find(
                (property: ConnectorPropertyInterface) => property.name === "account.multiattributelogin.handler.enable"
            ).value === "true";

        const property: ConnectorPropertyInterface = connector?.properties?.find(
        ...
        resolvedInitialValues = {
            ...
            email:
                isEnabled &&
                allowedAttributes?.includes(ClaimManagementConstants.EMAIL_CLAIM_URI) &&
    ...
    const getUpdatedConfigurations = (values: any) => {
        const defaultAllowedAttributes: string = ClaimManagementConstants.USER_NAME_CLAIM_URI;
    ...
    const resolveConnectorUpdateSuccessMessage = (): string => {
        return t("extensions:manage.accountLogin.alternativeLoginIdentifierPage.notification.success.description");
    };

    const resolveConnectorUpdateErrorMessage = (): string => {
        return t("extensions:manage.accountLogin.alternativeLoginIdentifierPage.notification.error.description");
    };
    ...
    const updateClaimProperties = (claim: Claim, checkedClaims: string[]) => {
        ...
        if (checkedClaims?.includes(claim.claimURI)) {
            ...
            isClaimUpdate = true;
            updatedClaimProperties = updatedClaimProperties.filter((property: Property) => property.key !== "isUnique");
        }
                ...
                dispatch(
                    addAlert({
                        description:
                            error?.description ||
                            t(
                                "extensions:manage.accountLogin.alternativeLoginIdentifierPage.claimUpdateNotification." +
                                    "error.description"
                            ),
                        level: AlertLevels.ERROR,
                        message:
        ...
        const checkedClaims: string[] = availableClaims
            .filter((claim: Claim) =>
                processedFormValues[claim?.displayName?.toLowerCase()] !== undefined
                    ? processedFormValues[claim?.displayName?.toLowerCase()]
                    : false
            )
    ...
    useEffect(() => {
        ...
        initializeForm();
    }, [availableClaims, connector, isLoading]);
        ...
        <>
            {!isLoading && initialFormValues ? (
                <PageLayout
                    ...
                    title={<>{t("extensions:manage.accountLogin.alternativeLoginIdentifierPage.pageTitle")}</>}
                    description={<>{t("extensions:manage.accountLogin.alternativeLoginIdentifierPage.description")}</>}
                    ...
                    <Grid className="mt-3">
                        <EmphasizedSegment className="form-wrapper" padded={"very"}>
                            <Form
                                ...
                                validate={null}
                                onSubmit={(values: AlternativeLoginIdentifierFormInterface) => handleSubmit(values)}
                            >
                                <Typography>
                                    {t(
                                        ...
                                        return (
                                            <>
                                                <ListItem
                                                    key={index}
                                                    disablePadding
                                                    disabled={!isAlphanumericUsername && name === "email"}
                                                >
                                                    <Field.OxygenCheckbox
                                                        ...
                                                        name={name}
                                                        required={false}
                                                        data-componentid={`${componentId}-${name}-checkbox`}
                                                    ...
                                                    <ListItemText
                                                        ...
                                                        secondary={
                                                            <Code className="alternative-login-identifier-code">
                                                                {claim?.claimURI}
                                                            </Code>
                                                        }
                                                    />
