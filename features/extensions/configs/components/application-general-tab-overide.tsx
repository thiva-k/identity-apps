...
import { DocumentationLink, EmphasizedSegment, Heading, PrimaryButton, ResourceTab } from "@wso2is/react-components";
...
export const ApplicationGeneralTabOverride: FunctionComponent<ApplicationGeneralTabOverridePropsInterface> = (
    ...
    const { appId, appName, clientId, ["data-componentid"]: componentId } = props;
        ...
        <ResourceTab.Pane controlledSegmentation>
            <EmphasizedSegment padded="very">
                <div className="form-container with-max-width">
                    <Heading bold as="h4">
                        Welcome to Asgardeo Try it!
                    </Heading>
                    <Heading as="h6">
                        Use the Try It application to experience different login flows on Asgardeo. Update the Sign-in
                        Methods and click{" "}
                        <Heading as="h6" weight="bold" inline>
                            {" "}
                            Try Login
                        </Heading>{" "}
                        to try out different login flows.
                        <DocumentationLink
                            link={"develop.applications.editApplication.asgardeoTryitApplication.general.learnMore"}
                            isLinkRef={true}
                        >
                            <Trans i18nKey={"extensions:common.learnMore"}>Learn More</Trans>
                        </DocumentationLink>
                    </Heading>
                    {TryItApplicationConstants.TRY_IT_TOUR_ENABLED && (
                        <>
                            <Divider hidden />
                            <PrimaryButton onClick={() => setIsTriggered(true)}>Take a Tour?</PrimaryButton>
                        </>
                    )}
                    <Joyride
                        continuous
