...
export const ValidationConfigEditPage: FunctionComponent<MyAccountSettingsEditPage> = (
    ...
    const isPasswordInputValidationEnabled: boolean = useSelector(
        (state: AppState) => state?.config?.ui?.isPasswordInputValidationEnabled
    );
    ...
    useEffect(() => {
        if (
            ...
            (!isPasswordInputValidationEnabled && !legacyPasswordPolicies && legacyPasswordPolicies?.length > 0)
        ) {
            return;
        }

        initializeForm();
    }, [passwordHistoryCountData, validationData, passwordExpiryData, legacyPasswordPolicies]);
     ...
     */
    useEffect(() => {
        if (!ValidationConfigStatusFetchRequestError && !passwordHistoryCountError && !passwordExpiryError) {
            return;
        }
        ...
        dispatch(
            addAlert({
                description: t(
                    "console:manage.features.validation.fetchValidationConfigData" + ".genericError.description"
                ),
                level: AlertLevels.ERROR,
                message: t("console:manage.features.validation.fetchValidationConfigData" + ".genericError.message")
            })
        );
        ...
        if (!isPasswordInputValidationEnabled) {
            updatedInitialFormValues = legacyPasswordPolicies.reduce(
                (formValues: any, property: ConnectorPropertyInterface) => {
                    const encodedPropertyName: any = GovernanceConnectorUtils.encodeConnectorPropertyName(
                        property.name
                    );
            ...
            setIsLegacyPasswordPolicyEnabled(
                updatedInitialFormValues?.[
    ...
    const handleUpdateMyAccountData = (values: ValidationFormInterface): void => {
        ...
        Promise.all(promises)
            ...
            .catch((error: AxiosError) => {
                ...
                dispatch(
                    addAlert({
                        description: t("console:manage.features.validation.notifications.genericError.description"),
                        level: AlertLevels.ERROR,
        ...
        return (
            <>
                ...
                <div className="validation-configurations-form mt-3 mb-3">
                    <div className="criteria">
                        ...
                        <Field.Input
                            ...
                            inputType="number"
                            name={GovernanceConnectorUtils.encodeConnectorPropertyName("passwordPolicy.min.length")}
                            validation={(value: string, allValues: Record<string, unknown>): string | undefined => {
                                const numValue: number = parseInt(value);
                                const min: number =
                                    ValidationConfigConstants.VALIDATION_CONFIGURATION_FORM_FIELD_CONSTRAINTS
                                        .PASSWORD_MIN_VALUE;
                        ...
                        <Field.Input
                            ariaLabel="Minimum length of the password"
                            inputType="number"
                            name={GovernanceConnectorUtils.encodeConnectorPropertyName("passwordPolicy.max.length")}
                            validation={(value: string, allValues: Record<string, unknown>): string | undefined => {
                                const numValue: number = parseInt(value);
                ...
                <Field.Input
                    ...
                    inputType="text"
                    name={GovernanceConnectorUtils.encodeConnectorPropertyName("passwordPolicy.pattern")}
                    type="text"
                    width={12}
                    required
                    ...
                    listen={(value: string) => {
                        setCurrentValues({
                            ...currentValues,
                            [GovernanceConnectorUtils.encodeConnectorPropertyName("passwordPolicy.pattern")]: value
                        });
                    }}
                ...
                <Field.Input
                    ...
                    inputType="text"
                    name={GovernanceConnectorUtils.encodeConnectorPropertyName("passwordPolicy.errorMsg")}
                    type="text"
                    width={12}
                    required
                    placeholder={"Enter password pattern regex"}
                    ...
                    listen={(value: string) => {
                        setCurrentValues({
                            ...currentValues,
                            [GovernanceConnectorUtils.encodeConnectorPropertyName("passwordPolicy.errorMsg")]: value
                        });
                    }}
            ...
            <div className="validation-configurations-form">
                ...
                <div className={"criteria rule mt-3"}>
                    <Field.Input
                        ...
                        validation={(value: string, allValues: Record<string, unknown>): string | undefined => {
                            const numValue: number = parseInt(value);
                            const min: number =
                                ValidationConfigConstants.VALIDATION_CONFIGURATION_FORM_FIELD_CONSTRAINTS.MIN_VALUE;
                        ...
                        min={ValidationConfigConstants.VALIDATION_CONFIGURATION_FORM_FIELD_CONSTRAINTS.MIN_VALUE}
                        max={
                            currentValues.minLength
                                ? currentValues.minLength
                                : ValidationConfigConstants.VALIDATION_CONFIGURATION_FORM_FIELD_CONSTRAINTS
                        ...
                        minLength={ValidationConfigConstants.VALIDATION_CONFIGURATION_FORM_FIELD_CONSTRAINTS.MIN_LENGTH}
                        readOnly={isReadOnly}
                        disabled={false}
                ...
                <div className="criteria rule">
                    <Field.Input
                        ...
                        maxLength={ValidationConfigConstants.VALIDATION_CONFIGURATION_FORM_FIELD_CONSTRAINTS.MIN_LENGTH}
                        minLength={
                            ValidationConfigConstants.VALIDATION_CONFIGURATION_FORM_FIELD_CONSTRAINTS
                                .PASSWORD_MIN_LENGTH
                        }
                ...
                <div className="criteria">
                    ...
                    <Field.Input
                        ...
                        placeholder={"min"}
                        value={
                            Number(initialFormValues.minUniqueCharacters) > 0
    ...
    return (
        <PageLayout
            ...
            title={<>{t("console:manage.features.validation.pageTitle")}</>}
            description={
                <>
                    {t("console:manage.features.validation.description")}
                    <DocumentationLink link={getLink("manage.validation.passwordValidation.learnMore")}>
                        {t("common:learnMore")}
                    </DocumentationLink>
                </>
            }
                                    ...
                                    {!isLoading ? (
                                        <Form
                                            ...
                                            onSubmit={(values: ValidationFormInterface) =>
                                                handleUpdateMyAccountData(values)
                                            }
