...
export const AddRoleUsers: FunctionComponent<AddRoleUserProps> = (props: AddRoleUserProps): ReactElement => {
    ...
    const getList = (limit: number, offset: number, filter: string, attribute: string, userStore: string) => {
        getUsersList(limit, offset, filter, attribute, userStore).then((response: UserListInterface) => {
            ...
            if (assignedUsers && assignedUsers.length !== 0) {
                ...
                if (responseUsers && responseUsers instanceof Array) {
                    ...
                    const unselectedUsers: UserBasicInterface[] = differenceBy([...responseUsers], selectedUserList);
    ...
    const handleSearchFieldChange = (e: React.FormEvent<HTMLInputElement>, { value }: { value: string }) => {
        ...
        if (!isEmpty(value)) {
            ...
            usersList &&
                usersList.map((user: UserBasicInterface) => {
    ...
    const addUser = () => {
        ...
        setUsersList(usersList.filter((user: UserBasicInterface) => checkedUnassignedListItems.indexOf(user) === -1));
    ...
    const addNewUserModal = () => (
        ...
        >
            <Modal.Header>
                {t("console:manage.features.roles.addRoleWizard.users.assignUserModal.heading", { type: "Role" })}
                                                        ...
                                                        if (user.orgId) {
                                                            return (
                                                                <Table.Row key={user.value}>
                                                                    ...
                                                                    <Table.Cell>{user.display}</Table.Cell>
                                                                    <Table.Cell>
                                                                        {user.orgId ===
                                                                        OrganizationManagementConstants.SUPER_ORGANIZATION_ID
                                                                            ? productName
                                                                            : user.orgName}
                                                                    </Table.Cell>
                                                                </Table.Row>
                                                            ...
                                                            const selectedUser: UserBasicInterface = selectedUsers?.find(
                                        ...
                                        <EmptyPlaceholder
                                            ...
                                            action={
                                                !isReadOnly && (
                                                    ...
                                                    >
                                                        {t(
                                                            "console:manage.features.roles.edit." +
                    ...
                    <Grid>
                        <Grid.Row columns={2}>
                            <TransferComponent
                                ...
                                searchPlaceholder={t(
                                    "console:manage.features.roles.addRoleWizard.users." +
                            ...
                            >
                                <TransferList
                                    ...
                                    listHeaders={[
                                        t(
                                            "console:manage.features.roles.addRoleWizard.users.assignUserModal." +
                                                "list.listHeader"
                                        )
                                    ]}
                                    handleHeaderCheckboxChange={selectAllUnAssignedList}
                                    ...
                                    emptyPlaceholderContent={t(
                                        "console:manage.features.transferList.list." +
                                            "emptyPlaceholders.roles.unselected",
                                        { type: "users" }
                                    )}
                                    emptyPlaceholderDefaultContent={t(
                                        "console:manage.features.transferList.list." + "emptyPlaceholders.default"
