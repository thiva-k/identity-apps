 ...
 */
export const UpdatedRolePermissionDetails: FunctionComponent<RolePermissionDetailProps> = (
    props: RolePermissionDetailProps
): ReactElement => {
    const { isReadOnly, role, onRoleUpdate, tabIndex, ["data-componentid"]: componentId } = props;
    ...
    const shouldFetchAPIResources: boolean =
        role?.audience?.type?.toUpperCase() === RoleAudienceTypes.ORGANIZATION && !isSubOrganization();
    ...
    useEffect(() => {
        !isReadOnly ? getExistingAPIResources() : null;
    }, [role]);
     ...
     */
    useEffect(() => {
        if (selectedAPIResourceFetchRequestError || authorizedAPIListForApplicationError) {
            dispatch(
                addAlert({
                    description: t(
    ...
    useEffect(() => {
        ...
        if (role.audience.type.toUpperCase() === RoleAudienceTypes.APPLICATION) {
            ...
            authorizedAPIListForApplication?.map((api: AuthorizedAPIListItemInterface) => {
                if (
                    !selectedAPIResources.find(
                        (selectedAPIResource: APIResourceInterface) => selectedAPIResource?.id === api?.id
                    )
                ) {
        ...
        if (role.audience.type.toUpperCase() === RoleAudienceTypes.ORGANIZATION) {
            ...
            const filteredOptions: DropdownItemProps[] = options.filter(
                (option: DropdownItemProps, index: number, self: DropdownItemProps[]) =>
                    index === self.findIndex((t: DropdownItemProps) => t.key === option.key)
            );
    ...
    const getAPIResourceById = async (initialAPIResourceIds: string[]): Promise<void> => {
        ...
        getAPIResourceDetailsBulk(apiResourceIds)
            ...
            .catch(() => {
                handleAlerts({
                    description: t(
                        "console:manage.features.roles.notifications." + "fetchAPIResource.error.description"
                    ),
                    level: AlertLevels.ERROR,
        ...
        <Grid container direction="column" justifyContent="center" alignItems="flex-start" spacing={2}>
            <Grid xs={8}>
                <Autocomplete
                    ...
                    isOptionEqualToValue={(option: DropdownProps, value: DropdownProps) => option.value === value.value}
                    ...
                    options={allAPIResourcesDropdownOptions
                        ...
                        .sort((a: DropdownItemProps, b: DropdownItemProps) => -b?.type?.localeCompare(a?.type))}
    ...
    const readOnlyPermissionList = (): ReactNode => (
        <Autocomplete
            ...
            renderInput={(params: AutocompleteRenderInputParams) => (
                <TextField {...params} data-componentid={`${componentId}-textfield`} />
            )}
            renderTags={(value: RolePermissionInterface[], getTagProps: AutocompleteRenderGetTagProps) =>
    ...
    return (
        <EmphasizedSegment padded="very">
            <Grid xs={8}>
                <Heading as="h4">{t("console:manage.features.roles.edit.permissions.heading")}</Heading>
                {isReadOnly ? (
