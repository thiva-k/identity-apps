...
export const RoleUsersList: FunctionComponent<RoleUsersPropsInterface> = (
    ...
    const { role, onRoleUpdate, isReadOnly, tabIndex } = props;
    ...
    const [selectedUserStoreDomainName, setSelectedUserStoreDomainName] = useState<string>("Primary");
    ...
    const isUserBelongToSelectedUserStore = (user: UserBasicInterface, userStoreName: string) => {
        ...
        return (
            (userNameChunks.length === 1 && userStoreName === "Primary") ||
    ...
    useEffect(() => {
        ...
        const alreadyAssignedUsersFromSelectedUserStore: UserBasicInterface[] =
            role?.users
                ?.map((user: RolesMemberInterface) => ({
                    id: user.value,
            ...
            const availableUserStoreList: UserstoreDisplayItem[] = [
                {
                    ...
                    name: t("console:manage.features.users.userstores." + "userstoreOptions.primary")
                }
            ];
    ...
    const getPlaceholders = () => {
        return (
            <EmptyPlaceholder
                subtitle={[t("console:manage.features.roles.edit.users.placeholders.emptyPlaceholder.subtitles.0")]}
                ...
                action={
                    !isReadOnly ? (
                        <PrimaryButton onClick={() => setIsPlaceholderVisible(false)}>
                            <Icon name="plus" />
                            {t("console:manage.features.roles.edit.users.placeholders.emptyPlaceholder" + ".action")}
                        </PrimaryButton>
                    ) : null
                }
            />
    ...
    const handleUsersUpdate: () => void = () => {
        ...
        const selectedUsers: UserBasicInterface[] = Object.keys(selectedAllUsers)
            .map((userStoreName: string) => {
                return selectedAllUsers[userStoreName];
            })
            .flat();

        // Formatting unassigned users list.
        const removedUsers: RolesMemberInterface[] =
            role?.users?.filter((user: RolesMemberInterface) => {
                return (
                    selectedUsers?.find((selectedUser: UserBasicInterface) => selectedUser.id === user.value) ===
                    undefined
                );
            }) ?? [];
        ...
        const addedUsers: UserBasicInterface[] =
            selectedUsers?.filter((user: UserBasicInterface) => {
                return (
                    role?.users?.find((selectedUser: RolesMemberInterface) => selectedUser.value === user.id) ===
                    undefined
                );
            }) ?? [];
            ...
            .catch((error: AxiosError) => {
                if (error?.response?.data?.detail) {
                    dispatch(
                        addAlert({
                            description: t(
                                "console:manage.features.roles.edit.groups.notifications.error.description",
                    ...
                    dispatch(
                        addAlert({
                            description: t(
                                "console:manage.features.roles.edit.groups.notifications.genericError" + ".description"
                            ),
                            level: AlertLevels.ERROR,
    ...
    return (
        <EmphasizedSegment padded="very" className="edit-role users">
            <Heading as="h4">{t("console:manage.features.roles.edit.users.heading")}</Heading>
                                    ...
                                    >
                                        {isUserStoresLoading ? (
                                            ...
                                            availableUserStores?.map((userstore: UserStoreListItem) => (
                                                <MenuItem key={userstore.name} value={userstore.name}>
                                                    {userstore.name}
                                                </MenuItem>
                                            ))
                                        )}
                                ...
                                <Autocomplete
                                    ...
                                    renderInput={(params: AutocompleteRenderInputParams) => (
                                        <TextField
                                            {...params}
                                            placeholder={t(
                                                "console:manage.features.roles.edit.users" +
