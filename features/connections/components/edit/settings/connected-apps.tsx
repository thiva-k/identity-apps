...
export const ConnectedApps: FunctionComponent<ConnectedAppsPropsInterface> = (
                ...
                dispatch(
                    addAlert({
                        description:
                            error?.description ||
                            t(
                                "console:develop.features.applications.notifications.fetchTemplates.genericError.description"
                            ),
                        level: AlertLevels.ERROR,
                        message:
                            error?.message ||
                ...
                if (appName === ApplicationManagementConstants.CONSOLE_APP_NAME) {
                    ...
                    history.push({
                        pathname: AppConstants.getPaths()
                            ...
                            .replace(":id", appId),
                        search: `#tab=${ApplicationManagementConstants.MY_ACCOUNT_LOGIN_FLOW_TAB}`
                    });
    ...
    return (
        <EmphasizedSegment padded="very">
            <Heading as="h4">
                {t("console:develop.features.idp.connectedApps.header", { idpName: editingIDP.name })}
            </Heading>
            <Divider hidden />
            {connectedApps && (
                <Input
