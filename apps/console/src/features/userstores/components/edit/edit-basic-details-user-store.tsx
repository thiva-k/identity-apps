...
export const EditBasicDetailsUserStore: FunctionComponent<EditBasicDetailsUserStorePropsInterface> = (
    ...
    const { userStore, id, properties, readOnly, update, ["data-testid"]: testId } = props;
    ...
    const onSubmitHandler = (values: Map<string, FormValue>): void => {
        ...
        const requiredData: PatchData[] = properties?.required
    ...
    const handleUserstoreDisable = (event: any, data: CheckboxProps): void => {
        const name: string = properties?.required?.find((property: TypeProperty) => property?.name === DISABLED)?.name;
                            ...
                            <Grid.Column width={8}>
                                <Field
                                    ...
                                    requiredErrorMessage={t(
                                        "console:manage.features.userstores.forms.general." +
                                            "name.requiredErrorMessage"
                                    )}
                                    placeholder={t(
                                        "console:manage.features.userstores.forms.general.name" + ".placeholder"
                                    )}
                                    value={
                                ...
                                {userstoresConfig.userstoreEdit.basicDetails.showType &&
                                    id !== CONSUMER_USERSTORE_ID && (
                                        <Field
                                            label={t(
                                                "console:manage.features.userstores.forms.general." + "type.label"
                                            )}
                                            name="type"
                                            type="text"
                                            disabled
                            ...
                            <Grid.Column width={8}>
                                {id !== CONSUMER_USERSTORE_ID
                                    ? properties?.required?.map((property: TypeProperty, index: number) => {
                                          const isDisabledField: boolean =
                                              property.description.split("#")[0] === DISABLED;
                                ...
                                <Grid.Row columns={1}>
                                    <Grid.Column width={8}>
                                        {properties?.optional?.nonSql.map((property: TypeProperty, index: number) => {
                                            const name: string = property.description.split("#")[0];
                                            const isPassword: boolean =
                                                property.attributes.find((attribute: PropertyAttribute) => {
                                            ...
                                            return isPassword ? (
                                                <Field
                                                    ...
                                                    showPassword={t("common:showPassword")}
                                                    hidePassword={t("common:hidePassword")}
                                                    data-testid={`${testId}-form-non-sql-password-input-${property.name}`}
                                                    disabled={readOnly}
                                                />
                                            ) : toggle ? (
                                                <Field
                                                    ...
                                                    toggle
                                                    data-testid={`${testId}-form-non-sql-toggle-${property.name}`}
                                                    disabled={readOnly}
                                                />
                                            ) : (
                                                <Field
                                                    ...
                                                    data-testid={`${testId}-form-non-sql-text-input-${property.name}`}
                                                    disabled={readOnly}
                                                />
                                            );
                                        })}
