...
export const isCompareToLastPeriodFeatureEnabled = (): boolean => {
    ...
    return !featureConfig?.insights?.disabledFeatures?.includes(OrgInsightsFeature.COMPARE_WITH_LAST_PERIOD);
};
 ...
 */
const convertUsageRecordToJSON = (
    usageRecords: string[]
): Array<{
    date: string;
    usage: number;
...
export const getInsightsForSelectedPeriod = async (
    ...
    const formattedCurrentUsageRecords: Array<{ date: string; usage: number }> = convertUsageRecordToJSON(
        currentPeriodInsights.usageRecords
    );
    ...
    const sortedUsageData: {
        ...
        current: number;
        previous?: number;
    }[] = orderBy(usageData, (record: { date: string; current: number; previous?: number }) => new Date(record.date), [
        "asc"
    ]);
...
export function shouldDateBeShownInXAxis(index: number, duration: DurationOption): boolean {
    ...
    if (duration === DurationOption.LAST_14_DAYS && index % 2 === 0) {
        return true;
    }

    if (duration === DurationOption.LAST_30_DAYS && index % 4 === 0) {
        return true;
    }
}
