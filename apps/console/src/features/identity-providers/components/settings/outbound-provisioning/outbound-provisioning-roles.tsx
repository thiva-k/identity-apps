...
export const OutboundProvisioningRoles: FunctionComponent<OutboundProvisioningRolesPropsInterface> = (
    ...
    const { idpId, idpRoles, isReadOnly, onUpdate, ["data-testid"]: testId } = props;
    ...
    useEffect(() => {
        ...
        getRolesList(null)
            .then(response => {
                if (response.status === 200) {
                    ...
                    setRoleList(
                        allRole?.Resources?.filter(role => {
                            return !(
                                role.displayName.includes("Application/") || role.displayName.includes("Internal/")
                            );
                        })
                    );
                }
        ...
        setSelectedRoles(idpRoles.outboundProvisioningRoles === undefined ? [] : idpRoles.outboundProvisioningRoles);
    }, []);
    ...
    return (
        <Grid>
            <Grid.Row>
                <Grid.Column width={8}>
                    <Heading as="h5">
                        {t("idp:develop.features.authenticationProvider.forms.outboundProvisioningRoles" + ".heading")}
                    </Heading>
                </Grid.Column>
            </Grid.Row>
                ...
                <Grid.Column width={8}>
                    <Form className="outbound-provisioning-roles role-select-dropdown">
                        <Form.Select
                            ...
                            search
                            label={t(
                                "idp:develop.features.authenticationProvider.forms." + "outboundProvisioningRoles.label"
                            )}
                    ...
                    <Hint>{t("idp:develop.features.authenticationProvider.forms.outboundProvisioningRoles.hint")}</Hint>
