...
export const AuthenticatorSettings: FunctionComponent<IdentityProviderSettingsPropsInterface> = (
    ...
    const { identityProvider, isLoading, onUpdate, isReadOnly, loader: Loader, ["data-testid"]: testId } = props;
    ...
    const [availableTemplates, setAvailableTemplates] = useState<IdentityProviderTemplateInterface[]>(undefined);
    ...
    const addCallbackUrl = (values: FederatedAuthenticatorListItemInterface) => {
        const authenticator: FederatedAuthenticatorWithMetaInterface = availableAuthenticators.find(
            authenticator => identityProvider.federatedAuthenticators.defaultAuthenticatorId === authenticator.id
        );
    ...
    const handleGetFederatedAuthenticatorAPICallError = error => {
        ...
        dispatch(
            addAlert({
                description: t(
                    "idp:develop.features.authenticationProvider.notifications.getFederatedAuthenticator." +
                        "genericError.description"
                ),
                level: AlertLevels.ERROR,
                message: t(
    ...
    const filterTemplates = (): void => {
        ...
        const filteredTemplates = identityProviderTemplates?.filter(
            template =>
        ...
        moderatedManualModeOptions = moderatedManualModeOptions?.filter(
            a => !availableAuthenticatorIDs.includes(a?.authenticatorId)
        );
            ...
            // Checkbox which triggers the default state of authenticator.
            {
                ...
                label: t(
                    isDefaultAuthenticator
                        ? "idp:develop.features.authenticationProvider.forms.authenticatorAccordion.default.0"
                        : "idp:develop.features.authenticationProvider.forms.authenticatorAccordion.default.1"
                ),
            ...
            // Toggle Switch which enables/disables the authenticator state.
            {
                ...
                label: t(
                    authenticator.data?.isEnabled
                        ? "idp:develop.features.authenticationProvider.forms.authenticatorAccordion.enable.0"
                        : "idp:develop.features.authenticationProvider.forms.authenticatorAccordion.enable.1"
                ),
            ...
            <Grid>
                <Grid.Row>
                    <Grid.Column width={16} textAlign="right">
                        ...
                        >
                            <Icon name="add" />
                            {t("idp:develop.features.authenticationProvider.buttons.addAuthenticator")}
                        </PrimaryButton>
                    </Grid.Column>
                </Grid.Row>
                <Grid.Row>
        ...
        if (availableAuthenticators.length > 0) {
            ...
            return (
                <EmptyPlaceholder
                    ...
                    imageSize="tiny"
                    title={t(
                        "idp:develop.features.authenticationProvider.placeHolders." + "emptyAuthenticatorList.title"
                    )}
                    subtitle={[
                        t(
        ...
        <div className="authentication-section">
            ...
            {deletingAuthenticator && (
                ...
                >
                    <ConfirmationModal.Header data-testid={`${testId}-authenticator-delete-confirmation`}>
                        {t("idp:develop.features.authenticationProvider.confirmations." + "deleteAuthenticator.header")}
                    ...
                    >
                        {t(
                            "idp:develop.features.authenticationProvider.confirmations." + "deleteAuthenticator.message"
                        )}
                    </ConfirmationModal.Message>
                    <ConfirmationModal.Content data-testid={`${testId}-authenticator-delete-confirmation`}>
                        {t(
                            "idp:develop.features.authenticationProvider.confirmations." + "deleteAuthenticator.content"
                        )}
            ...
            {showAddAuthenticatorWizard && (
                <AuthenticatorCreateWizard
                    title={t("idp:develop.features.authenticationProvider.modals." + "addAuthenticator.title")}
                    subTitle={t("idp:develop.features.authenticationProvider.modals." + "addAuthenticator.subTitle", {
