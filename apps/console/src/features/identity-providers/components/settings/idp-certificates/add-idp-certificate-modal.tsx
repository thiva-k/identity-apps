...
export const AddIdpCertificateModal: FC<AddIdPCertificateModalV2Props> = (props): ReactElement => {
    const { ["data-componentid"]: testId, isOpen, onClose, currentlyEditingIdP, refreshIdP } = props;
    ...
    const onCertificateChange = (result: PickerResult<string | File>): void => {
        try {
            ...
            setSubmitShouldBeDisabled((!result.pastedContent || !result.file) && !result.serialized && !result.valid);
        } catch (error) {
            setAlert({
                ...
                <Grid>
                    <Grid.Row column={1}>
                        <Grid.Column mobile={8} tablet={8} computer={8}>
                            <LinkButton floated="left" onClick={onClose} data-testid={`${testId}-cancel-button`}>
                                {t("idp:cancel")}
                            </LinkButton>
                        </Grid.Column>
                        <Grid.Column mobile={8} tablet={8} computer={8}>
