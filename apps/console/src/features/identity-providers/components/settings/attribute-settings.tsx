...
export const AttributeSettings: FunctionComponent<AttributeSelectionPropsInterface> = (
    ...
    const [selectedClaimsWithMapping, setSelectedClaimsWithMapping] = useState<
        IdentityProviderCommonClaimMappingInterface[]
    >([]);

    // Selected provisioning claims.
    const [selectedProvisioningClaimsWithDefaultValue, setSelectedProvisioningClaimsWithDefaultValue] = useState<
        IdentityProviderCommonClaimMappingInterface[]
    >([]);
    ...
    useEffect(() => {
        ...
        if (isEmpty(selectedClaimsWithMapping)) {
            ...
            setSelectedProvisioningClaimsWithDefaultValue(
                selectedProvisioningClaimsWithDefaultValue.filter(
            ...
            setSelectedProvisioningClaimsWithDefaultValue(
                selectedProvisioningClaimsWithDefaultValue.filter(
                    (claimWithDefaultValue: IdentityProviderCommonClaimMappingInterface) =>
                        isClaimExistsInIdPClaims(claimWithDefaultValue, selectedClaimsWithMapping)
                )
            );
            ...
            if (
                isEmpty(
                    selectedClaimsWithMapping?.find(
            ...
            if (
                isEmpty(
                    selectedClaimsWithMapping?.find(
                        (element: IdentityProviderCommonClaimMappingInterface) =>
                            element.mappedValue === subjectClaimUri
                    )
                )
            ) {
                setSubjectClaimUri("");
    ...
    const handleAttributesUpdate = (): void => {
        ...
        if (
            !isEmpty(
                selectedClaimsWithMapping?.filter((element: IdentityProviderCommonClaimMappingInterface) =>
                    isEmpty(element.mappedValue)
                )
            )
        ) {
            canSubmit = false;
        ...
        claimConfigurations["userIdClaim"] = matchingLocalClaim
            ? matchingLocalClaim
            : ({ uri: subjectClaimUri } as IdentityProviderClaimInterface);
        ...
        if (isRoleMappingsEnabled) {
            ...
            if (!isEmpty(selectedClaimsWithMapping)) {
                ...
                claimConfigurations["roleClaim"] = matchingLocalClaim
                    ? matchingLocalClaim
                    : ({ uri: roleClaimUri } as IdentityProviderClaimInterface);
            } else {
        ...
        if (canSubmit) {
            ...
            handleAttributeSettingsFormSubmit(idpId, claimConfigurations, roleMapping, onUpdate).finally(() => {
            ...
            dispatch(
                addAlert({
                    description: t(
                        "idp:develop.features.idp.notifications." + "submitAttributeSettings.error.description"
                    ),
                    level: AlertLevels.WARNING,
                    message: t("idp:develop.features.idp.notifications.submitAttributeSettings.error.message")
                })
            );
        }
    };
            ...
            <Grid className="attributes-settings">
                <div className="form-container with-max-width">
                    ...
                    {selectedClaimsWithMapping && (
                        <UriAttributesSettings
                            dropDownOptions={createDropdownOption(
                                selectedClaimsWithMapping,
                                availableLocalClaims
                            ).filter((element: DropdownOptionsInterface) => !isEmpty(element))}
                    ...
                    {provisioningAttributesEnabled && selectedProvisioningClaimsWithDefaultValue && (
                        <AttributeSelection
                            attributeList={buildProvisioningClaimList(
                                selectedClaimsWithMapping,
                                availableLocalClaims
