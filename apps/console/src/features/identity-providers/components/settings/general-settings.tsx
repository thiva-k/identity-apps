...
export const GeneralSettings: FunctionComponent<GeneralSettingsInterface> = (
    ...
    const { data: idpList, isLoading: isIdPListRequestLoading, error: idpListError } = useIdentityProviderList();
        ...
        deleteIdentityProvider(editingIDP.id)
            .then(() => {
                dispatch(
                    addAlert({
                        description: t(
                            "idp:develop.features.authenticationProvider.notifications.deleteIDP." +
                                "success.description"
                        ),
                        level: AlertLevels.SUCCESS,
                        message: t(
                            "idp:develop.features.authenticationProvider.notifications.deleteIDP." + "success.message"
                        )
                    })
                );
        ...
        updateIdentityProviderDetails({ id: editingIDP.id, ...updatedDetails })
            .then(() => {
                dispatch(
                    addAlert({
                        description: t(
                            "idp:develop.features.authenticationProvider.notifications.updateIDP." +
                                "success.description"
                        ),
                        level: AlertLevels.SUCCESS,
                        message: t(
        ...
        <>
            ...
            {!IdentityProviderManagementConstants.DELETING_FORBIDDEN_IDPS.includes(editingIDP.name) && (
                <Show when={AccessControlConstants.IDP_EDIT || AccessControlConstants.IDP_DELETE}>
                    <DangerZoneGroup
                        sectionHeader={t("idp:develop.features.authenticationProvider." + "dangerZoneGroup.header")}
                    >
                        <Show when={AccessControlConstants.IDP_EDIT}>
                            <DangerZone
                                actionTitle={t(
                                    ...
                                    { state: editingIDP.isEnabled ? t("idp:disable") : t("idp:enable") }
                                )}
                                header={t(
                                    "idp:develop.features.authenticationProvider.dangerZoneGroup." +
                                        "disableIDP.header",
                                    { state: editingIDP.isEnabled ? t("idp:disable") : t("idp:enable") }
                                )}
                                subheader={
                        ...
                        <Show when={AccessControlConstants.IDP_DELETE}>
                            <DangerZone
                                ...
                                header={t(
                                    "idp:develop.features.authenticationProvider." + "dangerZoneGroup.deleteIDP.header"
                                )}
                                subheader={t(
                                    "idp:develop.features.authenticationProvider." +
