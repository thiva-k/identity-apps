...
export const JITProvisioningConfigurationsForm: FunctionComponent<JITProvisioningConfigurationFormPropsInterface> = (
    ...
    const { initialValues, onSubmit, useStoreList, isReadOnly, isSubmitting, ["data-testid"]: testId } = props;
    ...
    const updateConfiguration = (values: any): any => {
        return {
            ...
            scheme: values.get(JITProvisioningConstants.PROVISIONING_SCHEME_TYPE_KEY) ?? initialValues?.scheme,
            userstore:
                values.get(JITProvisioningConstants.PROVISIONING_USER_STORE_DOMAIN_KEY) ?? initialValues.userstore
    ...
    const supportedProvisioningSchemes = [
        {
            label: t(
                "idp:develop.features.authenticationProvider" + ".forms.jitProvisioning.provisioningScheme.children.0"
            ),
            value: SupportedJITProvisioningSchemes.PROMPT_USERNAME_PASSWORD_CONSENT
        },
        {
            label: t(
                "idp:develop.features.authenticationProvider" + ".forms.jitProvisioning.provisioningScheme.children.1"
            ),
            value: SupportedJITProvisioningSchemes.PROMPT_PASSWORD_CONSENT
        },
        {
            label: t(
                "idp:develop.features.authenticationProvider" + ".forms.jitProvisioning.provisioningScheme.children.2"
            ),
            value: SupportedJITProvisioningSchemes.PROMPT_CONSENT
        },
        {
            label: t(
                "idp:develop.features.authenticationProvider." + "forms.jitProvisioning.provisioningScheme.children.3"
            ),
    ...
    const ProxyModeConflictMessage = (
        ...
        >
            <Message
                ...
                content={
                    <>
                        JIT user provisioning should be enabled for external identity providers (connections) when there
                        are MFA mechanisms such as <Code>TOTP</Code> and <Code>Email OTP</Code> configured in an
                ...
                {identityProviderConfig?.jitProvisioningSettings?.enableJitProvisioningField?.show && (
                    <Grid.Row columns={1}>
                        <Grid.Column mobile={16} tablet={16} computer={7}>
                            ...
                            />
                            <Hint>
                                When enabled, users who log in with this identity provider will be provisioned to your
                                organization.
                            </Hint>
                            {!isJITProvisioningEnabled ? ProxyModeConflictMessage : <Fragment />}
                        </Grid.Column>
                    </Grid.Row>
                )}
