...
import { OutboundProvisioningConnectors } from "./steps/outbound-provisioning-connector-create-wizard-steps";
import {
    getOutboundProvisioningConnectorMetadata,
    getOutboundProvisioningConnectorsList,
    updateOutboundProvisioningConnector
} from "../../api";
import { getOutboundProvisioningConnectorWizard } from "../../configs/ui";
import {
...
export const OutboundProvisioningConnectorCreateWizard: FunctionComponent<OutboundProvisioningConnectorCreateWizardPropsInterface> = (
    ...
    useEffect(() => {
        ...
        let initialConnector: OutboundProvisioningConnectorListItemInterface = connectorList.find(
            connector => connector.connectorId === selectedId
        );
    ...
    const STEPS = [
        ...
        },
        {
            content: (
                <OutboundProvisioningSettings
                    ...
                    onSubmit={(values): void => handleWizardFormSubmit(values, WizardStepsFormTypes.CONNECTOR_DETAILS)}
        ...
        },
        {
            ...
            title: t(
                "idp:develop.features.authenticationProvider." + "wizards.addProvisioningConnector.steps.summary.title"
            )
        }
    ];
    ...
    return (
        ...
        >
            <Modal.Header className="wizard-header" data-testid={`${testId}-modal-header`}>
                {t("idp:develop.features.authenticationProvider.modals.addProvisioningConnector.title")}
                <Heading as="h6">
                    {t("idp:develop.features.authenticationProvider.modals.addProvisioningConnector.subTitle")}
                </Heading>
            </Modal.Header>
            <Modal.Content className="steps-container" data-testid={`${testId}-modal-content-1`}>
                <Steps.Group
                    header={t(
                        "idp:develop.features.authenticationProvider.wizards." + "addProvisioningConnector.header"
                    )}
