 ...
 */
interface EnterpriseIDPCreateWizardProps
...
export const EnterpriseIDPCreateWizard: FC<EnterpriseIDPCreateWizardProps> = (
    ...
    const isIdpNameAlreadyTaken = (userInput: string): boolean => {
        if (idpList?.length > 0) {
            return idpList
                .reduce(
                    (set: Set<string>, { name }: StrictIdentityProviderInterface) => set.add(name),
                    new Set<string>()
                )
                .has(userInput);
        }
    ...
    const isLoopBackCall = (value: string) => {
        return !(URLUtils.isLoopBackCall(value) && commonConfig?.blockLoopBackCalls)
            ? undefined
            : t("idp:develop.features.idp.forms.common." + "internetResolvableErrorMessage");
    };
    ...
    const handleFormSubmit = (values: IdentityProviderFormValuesInterface) => {
         ...
         */
        const { idp: identityProvider } = cloneDeep(
            template.subTemplates.find(({ id }: IdentityProviderInterface) => {
                return (
                    id ===
        ...
        if (selectedProtocol === "oidc") {
            identityProvider.templateId = template.subTemplates.find(
        ...
        createIdentityProvider(identityProvider)
            ...
            .catch((error: AxiosError) => {
                ...
                if (
                    ...
                    setAlert({
                        description:
                            "You are trying to add a provider with an existing Identity" +
        ...
        <WizardPage
            validate={(values: any) => {
                ...
                if (!FormValidation.isValidResourceName(values.name)) {
                    errors.name = t(
                        "idp:develop.features.authenticationProvider." + "templates.enterprise.validation.name"
                    );
                }
        ...
        return (
            <WizardPage
                validate={(values: IdentityProviderFormValuesInterface) => {
                    ...
                    errors.clientId = composeValidators(required, length(OIDC_CLIENT_ID_MAX_LENGTH))(values.clientId);
        ...
        >
            <Grid>
                <Grid.Row columns={1}>
                    <Grid.Column mobile={16} tablet={16} computer={10}>
                        <p>
                            <b>Mode of certificate configuration</b>
                        </p>
            ...
            {selectedProtocol === "oidc" && selectedCertInputType === "jwks" && (
                <>
                    ...
                    />
                    <Hint>
                        {config.ui.productName} will use this URL to obtain keys to verify the signed responses from
                        your external IdP
                    </Hint>
                </>
            )}
            {selectedCertInputType === "pem" && (
                <>
                    <FilePicker
                        ...
                        onChange={(result: PickerResult<string | File>) => {
                            ...
                            setNextShouldBeDisabled(
                                (result.pastedContent?.length > 0 || result.file) && !result.serialized && !result.valid
                            );
                        }}
                    ...
                    />
                    <Hint>
                        {config.ui.productName} will use this certificate to verify the signed responses from your
                        external IdP.
                    </Hint>
                </>
            )}
    ...
    const resolveWizardPages = (): Array<ReactElement> => {
        if (selectedProtocol === "oidc") {
            return [wizardCommonFirstPage(), oidcConfigurationPage(), certificatesPage()];
        } else {
            return [wizardCommonFirstPage(), samlConfigurationPage(), certificatesPage()];
        }
    };
    ...
    const resolveHelpPanel = () => {
        ...
        const subTemplate: IdentityProviderTemplateInterface = cloneDeep(
            template.subTemplates.find(({ id }: IdentityProviderTemplateInterface) => {
                return (
                    id ===
                    (selectedProtocol === "saml"
                        ? IdentityProviderManagementConstants.IDP_TEMPLATE_IDS.SAML
        ...
        return (
            <ModalWithSidePanel.SidePanel>
                ...
                >
                    <div className="help-panel-header-text">Help</div>
                </ModalWithSidePanel.Header>
    ...
    const resolveDocumentationLink = (): ReactElement => {
        ...
        return <DocumentationLink link={docLink}>{t("idp:learnMore")}</DocumentationLink>;
    };
    ...
    return (
        ...
        >
            <ModalWithSidePanel.MainPanel>
                ...
                {/*Modal body content*/}
                <React.Fragment>
                    <ModalWithSidePanel.Content className="steps-container" data-testid={`${testId}-modal-content-1`}>
