...
import { authorizeAPI, removeAuthorizedAPI } from "../../api/api-authorization";
 ...
 */
interface APIAuthorizationResourcesProps extends SBACInterface<FeatureConfigInterface>, IdentifiableComponentInterface {
    /**
     * Template ID.
     */
    templateId: string;
...
export const APIAuthorization: FunctionComponent<APIAuthorizationResourcesProps> = (
    ...
    const { templateId, readOnly, ["data-componentid"]: componentId } = props;
            ...
            if (!isShownError) {
                ...
                dispatch(
                    addAlert<AlertInterface>({
                        ...
                        message: t(
                            "extensions:develop.apiResource.notifications.getAPIResources" + ".genericError.message"
                        )
                    })
                );
            }
    ...
    useEffect(() => {
        setSubAPIResourcesSectionLoading(
            !readOnly && (isUpdateData || isAllAPIResourcesListLoading || isSubscribedAPIResourcesListLoading)
        );
