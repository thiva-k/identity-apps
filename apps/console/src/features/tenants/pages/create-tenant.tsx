...
const TenantCreationPage: FunctionComponent<TestableComponentInterface> = (
        ...
        checkDuplicateTenants(submissionValue?.tenantName)
            .then((response: AxiosResponse) => {
                ...
                if (response.status == 200) {
                    ...
                    setAlert({
                        description: t(
                            "extensions:manage.features.tenant.wizards.addTenant.forms" +
            ...
            .catch((error: AxiosError) => {
                if (error.response.status == 404) {
                    ...
                    setAlert({
                        description: t(
                            "extensions:manage.features.tenant.notifications.addTenant" + ".genericError.description"
                        ),
                        level: AlertLevels.ERROR,
                        message: t("extensions:manage.features.tenant.notifications.addTenant.genericError.message")
                    });
        ...
        addNewTenant(tenantName)
                ...
                if (response.status === 201) {
                    ...
                    dispatch(
                        addAlert<AlertInterface>({
                            description: t(
                                "extensions:manage.features.tenant.notifications.addTenant.success.description",
                                tenantName
                            ),
                    ...
                    setTenantLoaderText(
                        t("extensions:manage.features.tenant.wizards.addTenant." + "forms.loaderMessages.tenantSwitch")
                    );
            ...
            .catch((error: AxiosError) => {
                ...
                if (
                    ...
                    setAlert({
                        description: t(
                            "extensions:manage.features.tenant.notifications.addTenant." + "limitReachError.description"
                        ),
                        level: AlertLevels.ERROR,
    ...
    const renderTenantLengthValidation = (): ReactElement => {
        ...
        if (
            newTenantName.length >= TenantManagementConstants.FORM_FIELD_CONSTRAINTS.TENANT_NAME_MIN_LENGTH &&
    ...
    return (
        <PageLayout padded={false} contentTopMargin={false} className="tenant-creation-page pt-5">
            <Grid centered className="tenant-form-container">
                <Grid.Column mobile={16} tablet={16} computer={6} largeScreen={6} widescreen={6}>
                    ...
                    />
                    <Card fluid centered className="basic-card no-hover">
                        <Card.Content className="p-5">
                            ...
                            <Grid centered>
                                <Grid.Row>
                                    <div className="card-heading">
                                        <Heading as="h2">
                                            {t("extensions:manage.features.tenant." + "tenantCreationPrompt.heading")}
                                        </Heading>
                                    </div>
                                </Grid.Row>
                                <Grid.Row>
                                    <div>
                                        ...
                                        <Text display="inline">
                                            <Flag name={String(deploymentRegion)?.toLowerCase() as FlagNameValues} />
                                            {deploymentRegion}{" "}
                                        </Text>
                                        <Text muted display="inline">
                                            {t(
                                        ...
                                        <Text muted display="inline">
                                            {t(
                                                "extensions:manage.features.tenant." +
                                                    "tenantCreationPrompt.subHeading6"
                                            )}
                                        </Text>
                                        <Text display="inline">
                                            <Flag name={String(alternativeRegion)?.toLowerCase() as FlagNameValues} />
                                            {alternativeRegion}{" "}
                                        </Text>
                                        <Text muted display="inline">
                                            {t(
                            ...
                            >
                                <Field.Input
                                    ...
                                    name="tenantName"
                                    label={t(
                                        "extensions:manage.features.tenant.wizards.addTenant" +
                            ...
                            {isCheckingTenantExistence ? (
                                <Text className="tenant-uri-prefix">
                                    {`${regionQualifiedConsoleUrl ?? "https://console.asgardeo.io"}/${tenantPrefix ??
                                        "t"}/`}
                            ...
                            <Grid className="m-0 pt-2">
                                <Grid.Row className="p-0">
                                    ...
                                    <Text muted>Must begin with an alphabet character</Text>
                                </Grid.Row>
                                <Grid.Row className="p-0">
                                    {renderTenantLengthValidation()}
                                    <Text muted>More than 4 characters, Less than 30 characters</Text>
                                </Grid.Row>
                                <Grid.Row className="p-0">
                                    {renderTenantAlphanumericValidation()}
                                    <Text muted>Only consist of lowercase alphanumerics</Text>
                                </Grid.Row>
                                <Grid.Row className="p-0">
                                    {renderTenantUniqueValidation()}
                                    <Text muted>Must be unique</Text>
                                </Grid.Row>
                            </Grid>
                            {isNewTenantLoading && <ContentLoader text={tenantLoaderText} />}
                        </Card.Content>
                        <Card.Content>
