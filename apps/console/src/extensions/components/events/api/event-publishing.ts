...
export const useEventConfig = <
    Data = EventPublishingAPIResponseInterface[],
...
export const updateEventConfigurations = (
    ...
    return httpClient(requestConfig)
        .then((response: AxiosResponse) => {
            if (response.status !== 200 && response.status !== 201) {
                throw new IdentityAppsApiException(
                    EventManagementConstants.ErrorMessages.EVENT_CONFIGURATION_UPDATE_INVALID_STATUS_CODE_ERROR.getErrorMessage(),
                    null,
                    response.status,
                    response.request,
                    response,
        ...
        .catch((error: AxiosError) => {
            let errorMessage: string = EventManagementConstants.ErrorMessages.EVENTS_CONFIGURATION_UPDATE_ERROR.getErrorMessage();

            if (error.response?.data?.code === EventManagementConstants.EVENT_UPDATE_ACTIVE_SUBS_ERROR_CODE_BE) {
                errorMessage = EventManagementConstants.ErrorMessages.EVENT_CONFIGURATION_UPDATE_ACTIVE_SUBS_ERROR.getErrorMessage();
            }
            throw new IdentityAppsApiException(
                errorMessage,
                error.stack,
