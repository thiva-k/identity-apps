...
import { deleteUser, getUserDetails, updateUserInfo } from "../../../../../features/users/api";
...
export const ConsumerUserProfile: FunctionComponent<ConsumerUserProfilePropsInterface> = (
    ...
    useEffect(() => {
        ...
        getUserDetails(user?.id, attributes).then((response: ProfileInfoInterface) => {
                ...
                if (schemaNames.length === 1) {
                    if (schemaNames[0] === "emails") {
                        if (ProfileUtils.isStringArray(userInfo[schemaNames[0]])) {
                            const emails: string[] | MultiValueAttributeInterface[] = userInfo[
                                schemaNames[0]
                            ] as string[];
                        ...
                        if (
                            schema.extended &&
                            userInfo[ProfileConstants.SCIM2_WSO2_CUSTOM_SCHEMA] &&
                    ...
                    if (schemaNames[0] === "name") {
                        ...
                        const subValue: SubValueInterface =
                            ...
                            userInfo[schemaNames[0]].find(
                                (subAttribute: { type: string }) => subAttribute.type === schemaNames[1]
                            );
    ...
    const handleSubmit = (values: Map<string, string | string[]>): void => {
        ...
        const booleanAttributes: Set<string> = new Set(
            profileSchema.map((schema: ProfileSchemaInterface) =>
                schema.type.toUpperCase() === "BOOLEAN" ? schema.name : null
            )
        );
        ...
        profileSchema.forEach((schema: ProfileSchemaInterface) => {
            ...
            if (schema.name !== "roles.default") {
                if (values.get(schema.name) !== undefined && values.get(schema.name).toString() !== undefined) {
                    if (
                        ProfileUtils.isMultiValuedSchemaAttribute(profileSchema, schemaNames[0]) ||
                        ...
                        if (schemaNames.length === 1) {
                            if (schema.extended) {
                                const customSchema: ScimOperationsInterface = data.Operations.find(
                                    (operation: ScimOperationsInterface) => operation.value[schema.schemaId]
                                );
                                ...
                                opValue =
                                    schemaNames[0] === UserManagementConstants.SCIM2_SCHEMA_DICTIONARY.get("EMAILS")
        ...
        updateUserInfo(user.id, data)
            .then(() => {
                onAlertFired({
                    ...
                    level: AlertLevels.SUCCESS,
                    message: t("console:manage.features.user.profile.notifications.updateProfileInfo.success.message")
                });
                ...
                if (error?.response?.data?.detail || error?.response?.data?.description) {
                    dispatch(
                        addAlert({
                            ...
                            level: AlertLevels.ERROR,
                            message: t(
                                "console:manage.features.user.profile.notifications.updateProfileInfo." +
                                    "error.message"
                            )
                        })
                    );
    ...
    const handleDangerZoneToggles = (toggleData: CheckboxProps) => {
        ...
        if (toggleData?.target?.checked) {
            ...
            setLoading(true);
            handleDangerActions(toggleData?.target?.id, toggleData?.target?.checked).finally(() => {
                setLoading(false);
                ...
                onAlertFired({
                    description:
                        attributeName === UserManagementConstants.SCIM2_ATTRIBUTES_DICTIONARY.get("ACCOUNT_LOCKED")
                            ? attributeValue
                                ? t(
                                      "console:manage.features.user.profile.notifications.lockUserAccount." +
                                          "success.description"
                                  )
                                : t(
                                      "console:manage.features.user.profile.notifications.unlockUserAccount." +
                                          "success.description"
                                  )
                            : attributeValue
                            ? t(
                                  "console:manage.features.user.profile.notifications.disableUserAccount." +
                                      "success.description"
                              )
                            : t(
                                  "console:manage.features.user.profile.notifications.enableUserAccount." +
                                      "success.description"
                              ),
                    level: AlertLevels.SUCCESS,
                    message:
            ...
            .catch((error: IdentityAppsApiException) => {
                if (error.response && error.response.data && error.response.data.description) {
                    onAlertFired({
                        ...
                        message:
                            attributeName === UserManagementConstants.SCIM2_ATTRIBUTES_DICTIONARY.get("ACCOUNT_LOCKED")
                                ? t(
                                      "console:manage.features.user.profile.notifications.lockUserAccount.error." +
                                          "message"
                                  )
                                : t(
                                      "console:manage.features.user.profile.notifications.disableUserAccount.error." +
                                          "message"
                                  )
                    });
                ...
                onAlertFired({
                    description:
                        ...
                        UserManagementConstants.SCIM2_ATTRIBUTES_DICTIONARY.get("ACCOUNT_LOCKED")
                            ? t(
                                  "console:manage.features.user.profile.notifications.lockUserAccount.genericError." +
                                      "description"
                              )
                            : t(
                                  "console:manage.features.user.profile.notifications.disableUserAccount.genericError." +
                                      "description"
                              ),
                    level: AlertLevels.ERROR,
                    message:
        ...
        return (
            <Show when={AccessControlConstants.USER_EDIT}>
                <DangerZone
                    ...
                    actionTitle={t(
                        "console:manage.features.user.editUser.dangerZoneGroup." + "lockUserZone.actionTitle"
                    )}
    ...
    const resolveDangerActions = (): ReactElement => {
        ...
        return (
            <Show when={AccessControlConstants.USER_DELETE || AccessControlConstants.USER_EDIT}>
                <DangerZoneGroup sectionHeader={t("console:manage.features.user.editUser.dangerZoneGroup.header")}>
                    {!isReadOnly && !isReadOnlyUserStore && user.userName !== ADMIN_USER_NAME && (
                        <DangerZone
                            data-testid={`${testId}-change-password`}
                            actionTitle={t(
                                "console:manage.features.user.editUser.dangerZoneGroup." +
                                    "passwordResetZone.actionTitle"
                            )}
                            header={t(
                                "console:manage.features.user.editUser.dangerZoneGroup." + "passwordResetZone.header"
                            )}
                            subheader={t(
                                "console:manage.features.user.editUser.dangerZoneGroup." + "passwordResetZone.subheader"
                            )}
                            onActionClick={(): void => {
                            ...
                            {!isReadOnly && configSettings?.accountDisable === "true" && (
                                <Show when={AccessControlConstants.USER_EDIT}>
                                    <DangerZone
                                        data-testid={`${testId}-danger-zone`}
                                        actionTitle={t(
                                            "console:manage.features.user.editUser." +
                                        ...
                                        toggle={{
                                            checked: accountDisable
                                                ...
                                                : user[ProfileConstants.SCIM2_WSO2_USER_SCHEMA]?.accountDisabled ===
                                                  "true",
                                            id: UserManagementConstants.SCIM2_ATTRIBUTES_DICTIONARY.get(
                                                "ACCOUNT_DISABLED"
                                            ),
                                            onChange: handleDangerZoneToggles
                                        }}
    ...
    const resolveFormField = (schema: ProfileSchemaInterface, fieldName: string, key: number): ReactElement => {
        if (schema.type.toUpperCase() === "BOOLEAN") {
                    ...
                    children={[
                        ...
                        countryList
                            ? countryList.map((list: DropdownItemProps) => {
                                  return {
                                      "data-testid": (`${testId}-profile-form-country-dropdown-` +
            ...
            const placeholder: string = `${t(
                "console:manage.features.user.editUser.dateOfBirth." + "placeholder.part1"
            )} ${fieldName} ${t("console:manage.features.user.editUser.dateOfBirth." + "placeholder.part2")}`;
                    ...
                    validation={(value: string, validation: Validation) => {
                        if (!moment(value, "YYYY-MM-DD", true).isValid()) {
                            ...
                            validation.errorMessages.push(
                                t("console:manage.features.users.forms.validation.dateFormatError", {
                                    field: fieldName
                                })
                            );
                        } else {
            ...
            return (
                <Field
                    ...
                    type="text"
                    value={
                        typeof profileInfo.get(schema.name) === "boolean"
                    ...
                    validation={(value: string, validation: Validation) => {
                        if (!RegExp(schema.regEx).test(value)) {
                            ...
                            if (schema.name === "phoneNumbers.mobile") {
                                validation.errorMessages.push(
                                    t("console:manage.features.users.forms.validation.mobileFormatError", {
                                        field: fieldName
                                    })
                                );
                            } else {
                                validation.errorMessages.push(
                                    t("console:manage.features.users.forms.validation.formatError", {
                                        field: fieldName
                                    })
                                );
                            }
                    ...
                    maxLength={
                        schema.name.toLowerCase().includes("url")
                            ? 1024
                            : schema.maxLength
                            ? schema.maxLength
    ...
    const generateProfileEditForm = (schema: ProfileSchemaInterface, key: number): JSX.Element => {
        ...
        const fieldName: string = t("console:manage.features.user.profile.fields." + schema.name.replace(".", "_"), {
            defaultValue: schema.displayName
        });

        const domainName: string[] = profileInfo
            ?.get(schema.name)
            ?.toString()
            .split("/");
            ...
            <Grid.Row columns={1} key={key}>
                <Grid.Column mobile={12} tablet={12} computer={6}>
                    ...
                    isEmpty(customUserAttributes?.userSourceId) ? (
                        ...
                        >
                            <label>
                                {usernameConfig?.enableValidator === "false" ? "Email (" + fieldName + ")" : fieldName}
                            </label>
                            <Input
                                data-testid={`${testId}-profile-form-${schema.name}-input`}
                                name={schema.name}
                ...
                !isEmpty(profileInfo) && !isUserProfileEmpty ? (
                                ...
                                {user.id && (
                                    <Grid.Row columns={1}>
                                        <Grid.Column mobile={12} tablet={12} computer={6}>
                                            <Form.Field>
                                                <label>
                                                    {t("extensions:manage.users." + "editUserProfile.userId")}
                                                </label>
                                                <Input name="userID" type="text" value={user.id} readOnly={true} />
                                            </Form.Field>
                                        </Grid.Column>
                                    </Grid.Row>
                                )}
                                    ...
                                    profileSchema.map((schema: ProfileSchemaInterface, index: number) => {
                                        if (
                                            !(
                                                ...
                                                (schema.name === "id" &&
                                                    schema.schemaId === "urn:ietf:params:scim:schemas:core:2.0")
                                            ) &&
                                ...
                                {createdDate && (
                                    <Grid.Row columns={1}>
                                        <Grid.Column mobile={12} tablet={12} computer={6}>
                                            <Form.Field>
                                                <label>
                                                    {t("console:manage.features.user.profile.fields." + "createdDate")}
                                                </label>
                                                <Input
                                                    name="createdDate"
                                                    type="text"
                                ...
                                {modifiedDate && (
                                    <Grid.Row columns={1}>
                                        <Grid.Column mobile={12} tablet={12} computer={6}>
                                            <Form.Field>
                                                <label>
                                                    {t("console:manage.features.user.profile.fields." + "modifiedDate")}
                                                </label>
                                                <Input
                                                    name="modifiedDate"
                                                    type="text"
                                    ...
                                    <Grid.Row columns={1}>
                                        <Grid.Column mobile={12} tablet={12} computer={6}>
                                            <Field
                                                ...
                                                label={t(
                                                    "console:manage.features.user." + "profile.fields.oneTimePassword"
                                                )}
                                                required={false}
                    ...
                    <EmphasizedSegment padded={false}>
                        <EmptyPlaceholder
                            ...
                            title={t(
                                "console:manage.features.user.profile.placeholders.userProfile." +
                ...
                <ConfirmationModal
                    ...
                    assertionHint={
                        <p>
                            <Trans
                                i18nKey={
                                    "console:manage.features.user.disableUser.confirmationModal." + "assertionHint"
                                }
                    ...
                    onPrimaryActionClick={() => {
                        setLoading(true);
                        handleDangerActions(editingAttribute.name, editingAttribute.value).finally(() => {
                            setLoading(false);
                            setShowDisableConfirmationModal(false);
                            setEditingAttribute(undefined);
                        });
